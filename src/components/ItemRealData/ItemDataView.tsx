...
export default class ItemDataView extends PureComponent {
            ...
            if (this.props.searchType !== nextProps.searchType || this.props.orgcode !== nextProps.orgcode || this.props.selectedDateVal !== nextProps.selectedDateVal) {
                if (nextProps.searchType === 'week') {
                    ...
                    this.setState({
                        ...
                        this.getItemCRKCount(selectedDateVal[0], selectedDateVal[1], '登记', nextProps.orgcode);
                        this.showCaseZKNumpie(selectedDateVal[0], selectedDateVal[1], nextProps.orgcode);
                        this.showCaseWpqspie('selectedDate', nextProps.orgcode,selectedDateVal[0], selectedDateVal[1] );
                    });
                }
            }
        }
              ...
              if (itemEchartwpqsBar && data) {
                  ...
                  if (data.list && data.list.length > 0) {
                    console.log('data---',data);
                      this.setState({
                          wpqsNoData: false,
                          typeLabel:qsTime === 'selectedDate'?typeLabel:qsTime === '6'||qsTime === '7'||qsTime === '8'?'周':'天',
                          qsTime:qsTime,
                      });
    ...
    showCaseEchartBar = () => {
        ...
        itemEchartpictorialBar.on('click', function(params) {
            ...
            that.props.changeToListPage({
                ...
                wpzt: that.state.showrkDataViewDJ ? '登记' :
                        that.state.showrkDataViewZK ? '在库' :
                          that.state.showrkDataViewDY ? '调用' :
                            that.state.showrkDataViewYS ? '移送' :
                              that.state.showrkDataViewCZ ? '处置' :
                                that.state.showrkDataViewYCCK ? '异常出库' : '',
            }, dataTime);
        });
    };
    // 入库出库各种状态切换
    changeRkListPageHeader = (zt) => {
        ...
        const { showrkDataViewZK,showrkDataViewDY,showrkDataViewYS,showrkDataViewCZ,showrkDataViewYCCK, TypeTime } = this.state;
        this.setState({
    ...
    render() {
        ...
        const { currentType, ZkwpData, ZkwpTotal, showrkDataViewDJ,showrkDataViewZK,showrkDataViewDY,showrkDataViewYS,showrkDataViewCZ,showrkDataViewYCCK, wpqsNoData, selectedDateData } = this.state;
        return (
                        ...
                        <div className={styles.cardBoxzk} style={{ padding: '0 5px' }}>
                            {ZkwpData.length > 0 ?
                                        ...
                                        <div>
                                            ...
                                            <div className={styles.progressCount}>
                                                <Tooltip title={item.name + ':' + item.count}>
                                                    <Progress percent={Math.round((item.count / ZkwpTotal) * 100)}
                                ...
                                <div style={{ padding: 16 }}>
                                    <div style={{ fontSize: 16, paddingTop: 2, color: 'rgba(0,0,0,0.85)' }}>在库物品数量展示
                                    </div>
                                    <div style={{
                                        height: '100%',
                                        width: '100%',
