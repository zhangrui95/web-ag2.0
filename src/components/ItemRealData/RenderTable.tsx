...
class RenderTable extends PureComponent {
  ...
  componentDidMount() {
    ...
    saveShare = (res, type, ajGzLx) => {
        ...
        if (type === 2) {
            let itemDetails = res;
            let detail = [`物品名称：${itemDetails && itemDetails.wpmc ? itemDetails.wpmc : ''}`, `物品种类：${itemDetails && itemDetails.wplx_mc ? itemDetails.wplx_mc : ''}`,
                `物品状态：${itemDetails && itemDetails.zt ? itemDetails.zt : ''}`, `库房信息：${itemDetails && itemDetails.szkf ? itemDetails.szkf : ''}`,
