...
export default class Statistics extends PureComponent {
    ...
    getZhTjSlByDwOfSecond = (code, rqType, current) => {// 获取下一级
        this.setState({
            loading: true,
        });
        let data_ks = '';
            ...
            callback: (res) => {
                if (res.error === null) {
                    if (res.data && res.data.list.length > 0) {
                        ...
                        if (getUserInfos().department === data[data.length - 1] || data[data.length - 1] !== code) {
                            data.push(code);
                        }
                        this.setState({
                            data: data,
    ...
    canPolice = (code, current) => { // 获取下一级疑似警情
        this.setState({
            loading: true,
        });
        let data_ks = moment(new Date()).add(-1, 'days').format('YYYY-MM-DD 08:00:00');
    ...
    getNext = (code, rqType) => {//点击获取下一级
        levelNum++;
        this.getNextLevelDeps(code, rqType);
    };
    getNextSecond = (code, rqType) => {//点击获取下一级
        levelNum++;
        this.getZhTjSlByDwOfSecond(code, rqType);
    };
    getNextPolice = (code, rqType) => {//点击获取下一级警情机构
        levelNum++;
        this.canPolice(code, rqType);
    };
    getNextLevelDeps = (code, rqType, current) => {//统计列表查询
        this.setState({
            loading: true,
        });
        this.props.dispatch({
    ...
    render() {
        let tjColumns = [{
            ...
            render: (text, res) => <div>
                {!res.xsajwt_num || res.xsajwt_num && res.xsajwt_num === 0 ?
                    <span className={styles.redNum} onClick={this.getNoList}>0</span> : <Link className={styles.redNum}
                                                                                              to={{
                ...
                {!res.xsaj_num || res.xsaj_num && res.xsaj_num === 0 ?
                    <span onClick={this.getNoList} className={styles.redAllNum}>/0</span> :
                    <Link className={styles.redAllNum} to={{
            ...
            render: (text, res) => <div>
                {!res.xzajwt_num || res.xzajwt_num && res.xzajwt_num === 0 ?
                    <span onClick={this.getNoList} className={styles.redNum}>0</span> : <Link className={styles.redNum}
                                                                                              to={{
            ...
            tjColumns.push({
                ...
                render: (text, res) => <div>
                    {!res.baqwt_num || res.baqwt_num && res.baqwt_num === 0 ?
                        <span onClick={this.getNoList} className={styles.redNum}>0</span> :
                        <Link className={styles.redNum} to={{
                ...
                render: (text, res) => <div>
                    {!res.wpwt_num || res.wpwt_num && res.wpwt_num === 0 ?
                        <span className={styles.redNum} onClick={this.getNoList}>0</span> :
                        <Link className={styles.redNum} to={{
                    ...
                    {!res.wp_num || res.wp_num && res.wp_num === 0 ?
                        <span className={styles.redAllNum} onClick={this.getNoList}>/0</span> :
                        <Link className={styles.redAllNum} to={{
        ...
        let colums = [{
            ...
            key: 'dw_name',
            align: 'left',
            render: (text, res) => <div className={styles.dwmcName} onClick={() => this.getNextPolice(res.dw_code)}>
                <Ellipsis tooltip length={20}>{text}</Ellipsis></div>,
        }, {
            title: '疑似警情',
            dataIndex: 'ysajjqsl',
            render: (text, res) => <div>
                {!text || (text && text === 0) ? <span className={styles.redAllNum} onClick={this.getNoList}>0</span> :
                    <Link className={styles.redAllNum}
                          to={{ pathname: '/ClearDispatching/DayPoliceClear', state: { res: res } }}>{text}</Link>}
            </div>,
        }];
        let tjColumns2 = [{
            title: '单位名称',
            ...
            align: 'left',
            width: 400,
            render: (text, res) => <div className={styles.dwmcName}
                                        onClick={() => this.getNextSecond(res.dw_code, this.state.rqType)}><Ellipsis
                tooltip length={30}>{text}</Ellipsis></div>,
        },
            {
                title: '警情总数',
                dataIndex: 'jq_num',
            ...
            },
            {
                ...
                render: (text, res) => <div>
                    {!text || (text && text === 0) ?
                        <span className={styles.redAllNum} onClick={this.getNoList}>0</span> :
                        <Link className={styles.redAllNum} to={{
                            pathname: this.state.rqType === '3' ? '/ClearDispatching/DayPoliceClear' : this.state.rqType === '6' ? '/ClearDispatching/MonthClear/MonthPoliceClear' : '/ClearDispatching/QuarterClear/QuarterPoliceClear',
        ...
        let m_data_ks = moment().subtract('days', 31).format('YYYY-MM-DD');
        let m_data_js = moment().subtract('days', 1).format('YYYY-MM-DD');
        let p_data_ks = moment().subtract('days', 90).format('YYYY-MM-DD');
        let P_data_js = moment().subtract('days', 1).format('YYYY-MM-DD');
        return (
                        ...
                        window.configUrl.clearHome ?
                            <Card className={styles.tableBox}
                                  title={<div className={styles.iconPerson}>
                                      ...
                                      <Radio.Group defaultValue="3" buttonStyle="solid" className={styles.redioGroup}
                                                   style={{ right: this.state.firstList ? '20px' : '45px' }}
                                                   onChange={this.getRadioChange}
                                                   disabled={this.state.loading ? true : false}>
                                          <Radio.Button value="3"><Tooltip placement="top"
                                                                           title='昨日'>日</Tooltip></Radio.Button>
                                          <Radio.Button value="6"><Tooltip placement="top"
                                                                           title={m_data_ks + '~' + m_data_js}>月</Tooltip></Radio.Button>
                                          <Radio.Button value="9"><Tooltip placement="top"
                                                                           title={(p_data_ks > '2019-06-01' ? p_data_ks : '2019-06-01') + '~' + P_data_js}>季</Tooltip></Radio.Button>
                                      </Radio.Group></div>}
                            >
                                <Table loading={this.state.loading} size="middle" columns={tjColumns2}
                                       dataSource={this.state.newData && this.state.newData.list ? this.state.newData.list : []}
                                       className={styles.homeTable} pagination={paginationPage}
                                       onChange={this.handleTableChange}/>
                            </Card> : <Tabs defaultActiveKey="tab1" type="card" onChange={this.changeTabs}>
                                {
                                    authorityIsTrue(userAuthorityCode.RIQING) ? (
                                        <TabPane tab={<div className={styles.iconPerson}><img
                                            ...
                                            <Card title={null}>
                                                <Table loading={this.state.loading} size="middle" columns={colums}
                                                       dataSource={this.state.newData && this.state.newData.list ? this.state.newData.list : []}
                                                       className={styles.homeTable} pagination={paginationPage}
                                                       onChange={this.handleTableChange}/>
                                            </Card>
                                ...
                                >
                                    <div>
                                        <Tooltip placement="top" title='返回'>
                                            <img src={back} className={styles.rollBack} onClick={() => this.getPop('1')}
                                                 style={{ display: this.state.firstList ? 'none' : 'block' }}/>
                                        </Tooltip>
                                        <Radio.Group value={this.state.rqType} defaultValue="3" buttonStyle="solid"
                                                     className={styles.redioGroup}
                                                     style={{ right: this.state.firstList ? '20px' : '45px' }}
                                                     onChange={this.getRadioChange}
                                                     disabled={this.state.loading ? true : false}>
                                            <Radio.Button value="3"><Tooltip placement="top"
                                                                             title='本周'>周</Tooltip></Radio.Button>
                                            <Radio.Button value="6"><Tooltip placement="top"
                                                                             title='本月'>月</Tooltip></Radio.Button>
                                            <Radio.Button value="9"><Tooltip placement="top"
