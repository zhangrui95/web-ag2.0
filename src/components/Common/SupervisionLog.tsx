            ...
            <div style={{ position: 'relative', left: '-30px', top: '10px', paddingBottom: '10px' }}>
                ...
                </p>
                {
                    Isdetail === '发起督办' ? (
                        <div>
                            <Button type='primary'
                                    onClick={() => this.seeDetail(true, Isdetail, NowDbrz)}>查看督办详情</Button>
                        </div>
                    ) : (
                        Isdetail === '处理反馈' && zrrMc && zrdwMc && clsj ? (
                            <div>
                                <Button type='primary'
                                        onClick={() => this.seeDetail(true, Isdetail, NowDbrz)}>查看反馈详情</Button>
                            </div>
                        ) : null
                    )
                }
                                ...
                                <Row style={{ margin: '0 -24px' }}>
                                    <Col md={6} sm={24}>
                                        ...
                                        <div className={listStyles.Indextail}
                                             style={{ paddingLeft: 96 }}>{detailData && detailData.wtlx ? detailData.wtlx : ''}</div>
                                    </Col>
                                    <Col md={6} sm={24}>
                                        <div className={listStyles.Indexfrom}>产生方式：</div>
                                        <div className={listStyles.Indextail}
                                             style={{ paddingLeft: 96 }}>{detailData && detailData.fxfs ? detailData.fxfs : ''}</div>
                                    </Col>
                                    <Col md={6} sm={24}>
                                        <div className={listStyles.Indexfrom}>责任人：</div>
                                        <div
                                            className={listStyles.Indextail}>{dbrz.length > 0 ? dbrz[0].bdbrMC : ''}</div>
                                    </Col>
                                    <Col md={6} sm={24}>
                                        <div className={listStyles.Indexfrom}>责任单位：</div>
                                        <div className={listStyles.Indextail}
                                             style={{ paddingLeft: 96 }}>{dbrz.length > 0 ? dbrz[0].bdbrDwmc : ''}</div>
                                    </Col>
                                </Row>
                                <div className={styles.superve} style={{ width: superveWidth }}>
                                    <Steps current={currentNum}>
                                        <Step title={<span style={{ fontSize: 14 }}>发起督办</span>}
                                              description={this.extraDescription(NowDbrz[0], '发起督办')}/>
                                        <Step title={<span style={{ fontSize: 14 }}>整改中</span>}
                                              description={this.extraDescription(NowDbrz[1], '整改中')}/>
                                        <Step title={<span style={{ fontSize: 14 }}>处理反馈</span>}
                                                ...
                                                <span style={{ float: 'right' }}>
                                                    {
                                                        detailData.dbList.length > 1 ? (
                                                            <span className={styles.recordList}
                                                                  onClick={() => this.SeeHistory(true, dbHistoryData)}>查看历史督办记录</span>
                                                        ) : null
                                                    }
                                                    {
                                                        isDb ? (
                                                            <span>
                                                                <Button
                                                                    ...
                                                                    style={{ marginLeft: 8 }}
                                                                    disabled={currentArr.length > 0 && (currentArr[currentArr.length - 1].dbzt === '30' && currentArr[currentArr.length - 1].fkzt === '1') ? false : true}
                                                                >
                                                                    再次督办
                                                                </Button>
                                                                <Button
                                                                    ...
                                                                    style={{ marginLeft: 8 }}
                                                                    disabled={currentArr.length > 0 && currentArr[currentArr.length - 1].dbzt === '30' ? false : true}
                                                                >
                                                                    确认整改完毕
                                                                </Button>
                                                            </span>
