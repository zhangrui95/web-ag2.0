                ...
                >
                    <div className={styles.box}>
                        <div className={styles.leftBox}>
                            <Row style={{height: '120px'}}>
                                ...
                                <Col span={19} className={styles.topDetail}>
                                    <Col span={24}>案件名称：<a
                                        onClick={() => this.openCaseDetail(this.state.recordKp.ajlx, this.state.recordKp.ajbh)}>{this.state.recordKp && this.state.recordKp.ajmc ? this.state.recordKp.ajmc : ''}</a></Col>
                                    <Col
                        ...
                        <div className={styles.rightBox}>
                            ...
                            <Radio.Group style={{marginBottom: 16, left: 24}} defaultValue={''}
                                         value={this.state.kpjlType} className={styles.redioGroup}
                                         onChange={this.getKpjl}>
                                <Radio.Button value="">全部</Radio.Button>
                                <Radio.Button
                                    value="0">{detail && detail.total_kf ? detail.total_kf : ''}</Radio.Button>
                                <Radio.Button
                                    value="1">{detail && detail.total_bf ? detail.total_bf : ''}</Radio.Button>
                                <Radio.Button
                                    value="2">{detail && detail.total_jf ? detail.total_jf : ''}</Radio.Button>
                            </Radio.Group>
                                ...
                                <Timeline>
                                    {
                                        detail && detail.kpJlList && detail.kpJlList.map((item) => {
                                            return <Timeline.Item
                                                color={item.xm_type === '0' ? '#f00' : item.xm_type === '1' ? '#ff6600' : item.xm_type === '2' ? '#0c0' : '#1890ff'}>
                                                <div>时间：{item.kpsj}</div>
                                                <div>详情：<span
                                                    style={{color: item.xm_type === '0' ? '#f00' : item.xm_type === '1' ? '#ff6600' : item.xm_type === '2' ? '#0c0' : '#1890ff'}}>{item.fz_lasted}</span><span
