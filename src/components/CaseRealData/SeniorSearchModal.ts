...
export default class SeniorSearchModal extends PureComponent {
  ...
  // 无法选择的日期
    ...
    foot = () => {
      return (
        <div>
          <Button style={{ color: '#2095FF', borderColor: '#2095FF' }} onClick={this.resetSearch}>重置</Button>
    ...
    render() {
        const { form: { getFieldDecorator },treeDefaultExpandedKeys,depTree,CaseStatusType} = this.props;
                        ...
                        <Col md={24} sm={24}>
                          <FormItem label="案件状态" labelCol={{ span: 8 }} wrapperCol={{ span: 16 }}>
                            {getFieldDecorator('ajzt', {
                              ...
                              <Select placeholder="请选择案件状态" style={{ width: '100%' }} mode={'multiple'} getPopupContainer={triggerNode => triggerNode.parentNode}>
                                {CaseStatusOption}
                              </Select>
                            )}
                          </FormItem>
