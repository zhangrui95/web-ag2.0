      ...
      <div style={{ width: superveWidth }} className={styles.superve}>
        <Steps current={this.currentNum(policeDetails)}>
          {policeDetails.sarq || policeDetails.sadw || policeDetails.sabar ? (
            <Step
              status={policeDetails.ajzt === '受理' ? 'process' : 'wait'}
              title={<span style={{ fontSize: 14 }}>受理</span>}
              description={this.extraDescriptionSa(
                policeDetails.sarq,
                policeDetails.sadw,
                policeDetails.sabar,
              )}
          ...
          {policeDetails.larq || policeDetails.ladw || policeDetails.labar ? (
            <Step
              status={policeDetails.ajzt === '立案' ? 'process' : 'wait'}
              title={<span style={{ fontSize: 14 }}>立案</span>}
              description={this.extraDescriptionLa(
                policeDetails.larq,
                policeDetails.ladw,
                policeDetails.labar,
              )}
          ...
          {policeDetails.parq || policeDetails.padw || policeDetails.pabar ? (
            <Step
              status={policeDetails.ajzt === '破案' ? 'process' : 'wait'}
              title={<span style={{ fontSize: 14 }}>破案</span>}
              description={this.extraDescriptionPa(
                policeDetails.parq,
                policeDetails.padw,
                policeDetails.pabar,
              )}
          ...
          {policeDetails.jarq || policeDetails.jadw || policeDetails.jabar ? (
            <Step
              status={policeDetails.ajzt === '结案' ? 'process' : 'wait'}
              title={<span style={{ fontSize: 14 }}>结案</span>}
              description={this.extraDescriptionJa(
                policeDetails.jarq,
                policeDetails.jadw,
                policeDetails.jabar,
              )}
          ...
          {policeDetails.xarq || policeDetails.xadw || policeDetails.xabar ? (
            <Step
              status={policeDetails.ajzt === '销案' ? 'process' : 'wait'}
              title={<span style={{ fontSize: 14 }}>销案</span>}
              description={this.extraDescriptionXa(
                policeDetails.xarq,
                policeDetails.xadw,
                policeDetails.xabar,
              )}
