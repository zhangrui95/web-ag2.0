...
export default class XzCaseEnforcementDataView extends PureComponent {
            ...
            if ((this.props.searchType !== nextProps.searchType) || this.props.orgcode !== nextProps.orgcode || this.props.selectedDateVal !== nextProps.selectedDateVal) {
                if (nextProps.searchType === 'week') {
                    ...
                    this.setState({
                        ...
                        this.getAdministrativePenalty(selectedDateVal[0], selectedDateVal[1], nextProps.orgcode);
    ...
    getAdministrativePenalty = (startTime, endTime, orgcode = this.props.orgcode) => {
        this.props.dispatch({
            ...
            callback: (data) => {
                if ((this.props.location && this.props.location.query && this.props.location.query.id) || (this.props.location.state && this.props.location.state.code && this.props.location.state.kssj && this.props.location.state.jssj)) {
                    return;
                } else {
                    if (data) {
                        if (data.list.length > 0) {
                            ...
                            xzCaseEchartBar.innerHTML = '<div style="height: 100%;width: 100%;">' +
    ...
    showXzCaseEchartRingPie = () => {
        ...
        xzCaseEchartRingPie.on('click', function(params) {
            ...
            that.props.changeToListPage({ ajzt: params.name && params.name === '受理' ? '201' : '206' }, dataTime, params.name);
        });
    };
    // 受结情况展示
    showXzCaseEchartLine = () => {
        ...
        xzCaseEchartLine.on('click', function(params) {
            ...
            that.props.changeToListPage({ ajzt: params.seriesName && params.seriesName === '受理' ? '201' : '206' }, dataTime,params.seriesName);
        });
    };
        ...
        caseTypeStatisticsBar.getZr().on('click', function(params) {
            ...
            if (caseTypeStatisticsBar.containPixel('grid', point)) {
                ...
                if (code) {
                    const { currentType } = that.state;
                    const dataTime = currentType === 'selectedDate' ? that.props.selectedDateVal : that.getTime(currentType);
                    that.props.changeToListPage({ ajlb: [code.substring(0, 2) + '0000', code.substring(0, 4) + '00', code] }, dataTime,'受理');
                }
            }
        });
    };
                        ...
                        <div className={styles.viewCount}>
                            ...
                            >
                                ...
                                <div className={styles.countButtonNumber}>
                                    <div>{nowDataName[0]}：{nowData[0]}</div>
                                    <div>{nowDataName[1]}：{nowData[1]}</div>
                                </div>
                            </div>
                            <div
                            ...
                            >
                                ...
                                <div className={styles.countButtonNumber}>
                                    <div>{lastDataName[0]}：{lastData[0]}</div>
                                    <div>{lastDataName[1]}：{lastData[1]}</div>
                                </div>
                            </div>
                            <div
                            ...
                            >
                                ...
                                <div className={styles.countButtonNumber}>
                                    <div>{beforeLastDataName[0]}：{beforeLastData[0]}</div>
                                    <div>{beforeLastDataName[1]}：{beforeLastData[1]}</div>
                                </div>
