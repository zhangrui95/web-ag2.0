...
let showModalEchart = false;

export default class BarGraphFullScreenModal extends PureComponent {
    state = {
        laDataArry: [],//立案
        ...
        comparedRq2_ks: '',
        comparedRq2_js: '',
        headCityName: '默认XX',//XX市各级公安机关受立案情况的XX
        xzrq_ks: '',//列表选择日期开始
        xzrq_js: '',
    ...
    componentDidMount() {
        ...
        if (comparedRq1_ks != null && comparedRq1_js != null && comparedRq2_js != null && comparedRq2_ks != null
            ...
            this.setState({
                ...
                if (parentId != null && parentId != '' && orgId != null && orgId != '' && levelState != null && levelState != '' && depth != null && depth != '') {
                    this.setState({
                        parentId: parentId,
                        orgId: orgId,
                ...
                } else {//这里打印的是最新的state值
                    //给AcceptAndRegisterList传值，用于放大柱状图的时候传值使用
                    if (parentId != null && parentId != '' && orgId != null && orgId != '' && levelState != null && levelState != '') {
                        this.setState({
                            parentId: parentId,
                            orgId: orgId,
                            levelState: levelState,
    ...
    getAcceptAndRegisterData = (param) => {
        ...
        this.props.dispatch({
            ...
            callback: (data) => {
                ...
                //为表格标题赋值
                this.setState({
                    headCityName: data.headCityName.toString(),
                });
                //判断上一级按钮是否显示
                if (levelState == 1) {
                    //返回到用户登录级别上一级按钮不显示
                ...
                if (echartBar) {
                    ...
                    if (isTimeCompared) {
                        ...
                        // echartBar = echarts.init(document.getElementById('BarGraphFullScreenModal'));
    ...
    showEchart = () => {
        const domId = document.getElementById('barGraphFullScreenModal');
        // console.log('domId',domId)
        if (domId != null && !showModalEchart) {
            showModalEchart = true;
            echartBar = echarts.init(domId);
            // alert(3)
            const option = {
                toolbox: {
                    show: false,
    ...
    onCancel = () => {
        showModalEchart = false;
        this.props.closeBarGraphFullScreenModal(false);
    };
                ...
                >
                    <Spin spinning={this.state.loading1}>
                        <Card bodyStyle={{ height: 750 }}>
                            ...
                            {/*:*/}
                            {/*<span style={{float:'left'}} className={styles.headCityName}>{headCityName !== '' ? `${headCityName}各级公安机关受立案情况` : ''}</span>*/}
                            {/*}*/}
                            {upperLevelButtonState ?
                                <Button type="primary" className={styles.forBigMenu}
