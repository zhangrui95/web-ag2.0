...
class RenderTable extends PureComponent {
    ...
    saveShare = (res, type, ajGzLx) => {
        this.setState({
            sx: (res.ajmc ? res.ajmc + '、' : '') + (res.salxMc ? res.salxMc + '、' : '') + (res.name ? res.name + '、' : '') + (res.wtlxMc ? res.wtlxMc + '、' : '') + (res.gjsj ? res.gjsj : ''),
            shareRecord: res,
        });
        if (type === 2) {
            this.setState({
    ...
    render() {
        const { data, UnareaData: { loading } } = this.props;
        let columns;
        columns = [
            {
                title: '告警时间',
        ...
        let detail = (
            <Row style={{ width: '90%', margin: '0 38px 10px', lineHeight: '36px', color: 'rgba(0, 0, 0, 0.85)' }}>
                ...
                <Col
                    span={6}>人员类型：{this.state.shareRecord && this.state.shareRecord.salxMc ? this.state.shareRecord.salxMc : ''}</Col>
                ...
                <Col span={6}>办案单位：<Tooltip
                    title={this.state.shareRecord && this.state.shareRecord.badwMc && this.state.shareRecord.badwMc.length > 7 ? this.state.shareRecord.badwMc : null}>{this.state.shareRecord && this.state.shareRecord.badwMc ? this.state.shareRecord.badwMc.length > 7 ? this.state.shareRecord.badwMc.substring(0, 7) + '...' : this.state.shareRecord.badwMc : ''}</Tooltip></Col>
                <Col
