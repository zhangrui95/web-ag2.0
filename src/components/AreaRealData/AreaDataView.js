...
import { Row, Col, Card, Divider, Tooltip, Button, Radio, Icon, message, Progress, Timeline } from 'antd';
...
const colors1 = ['#FE3265', '#CCFF36', '#259DF4', '#40537E', '#EDB59C', '#FED501', '#3074B5', '#72C4B8', '#CBFF30', '#FE0000', '#42C92E'];
...
export default class ItemDataView extends PureComponent {
            ...
            if (this.props.searchType !== nextProps.searchType || this.props.orgcode !== nextProps.orgcode || this.props.selectedDateVal !== nextProps.selectedDateVal) {
                if (nextProps.searchType === 'week') {
                    ...
                    this.setState({
                        ...
                        this.getAreaSpecialRYCFCount(selectedDateVal[0], selectedDateVal[1], nextProps.orgcode);
                        ...
                        this.getAreaRQRCQSCount('selectedDate', nextProps.orgcode, selectedDateVal[0], selectedDateVal[1]);
                    });
                }
            }
        }
    ...
    returnSaryrqrczs = (SARYRQRCdataLength, SARYRQRCdata, SARYRQRCTotal, chooseBaq) => {
        if (SARYRQRCdataLength === 1) {
            ...
            return (
                <div>
                    <div className={AreaDataViewStyles.IntoTitle}>| 办案区入区人次展示<a style={{ float: 'right' }} onClick={() => this.resetBaq()}>全部</a>
                    </div>
                    <div className={AreaDataViewStyles.IntoAreaName}>
                        {SARYRQRCdata && SARYRQRCdata.map((item) =>
                            <div className={AreaDataViewStyles.IntoPerson}>
                                <h5>{item.name}</h5>
                                <div>
                                    <Tooltip title={item.name + '入区人数：' + item.count1} placement="bottomLeft">
                                        <Progress percent={item.count1 / parseInt(SARYRQRCTotal) * 100}
                                                  status={item.orgid === chooseBaq && chooseBaq !== '' ? 'exception' : 'active'}
                                                  strokeWidth={16}
    ...
    render() {
        ...
        const { currentType, SpecialRYCFdata, NLFBdata, NLFBTotal, SARYRQRCdata, SARYRQRCdataLength, SARYRQRCTotal, nowData, lastData, beforeLastData, chooseBaq, selectedDateData } = this.state;
        return (
          <Card style={{ position: 'relative'}} className={styles.policeDataCard}>
            <div className={styles.AreaDataView} style={showDataView ? {} : { position: 'absolute', zIndex: -1 }}>
                {
                    currentType !== 'selectedDate' ? (
              ...
              <div style={{backgroundColor:'#252c3c',padding:'0 16px'}}>
                                    ...
                                    NLFBdata.length > 0 ?
                                        NLFBdata.map((item) =>
                                            <Row gutter={rowLayout} style={{ height: 44 }}>
                                                ...
                                                <Col sm={24} lg={22}>
                                                    <Tooltip title={item.name + '年龄段数量：' + item.count}
                                                             placement="bottomLeft">
                                                        <Progress percent={(item.count / NLFBTotal) * 100}
                                                                  strokeWidth={12}/>
                                                    </Tooltip>
                                                </Col>
                                            </Row>,
                ...
                <Row gutter={rowLayout} className={styles.listPageRow}>
                    <Col {...colLayout}>
                        <div className={styles.cardBox} style={{ padding: SARYRQRCdataLength > 1 ? 21 : 0 }}>
                            {/*<div className={styles.cardBoxTitle}>|  办案区入区人次展示</div>*/}
                            {this.returnSaryrqrczs(SARYRQRCdataLength, SARYRQRCdata, SARYRQRCTotal, chooseBaq)}
                        </div>
                    </Col>
                    <Col {...colLayout}>
                ...
                <Row gutter={rowLayout} className={styles.listPageRow}>
                    <Col {...colLayout1} style={{ marginBottom:32 }}>
                        ...
                        <div id="rqrcqszs" className={styles.cardBox}></div>
                    </Col>
                </Row>
              </div>
            </div>
