...
export default class Index extends PureComponent {
  ...
  exportData = () => {
    ...
    if ((rksjTime && rksjTime.length > 0) || (ljsjTime && ljsjTime.length > 0)) {
      const isAfterDate = rksjTime && rksjTime.length > 0 ? moment(formValues.rksj_js).isAfter(moment(formValues.rksj_ks).add(exportListDataMaxDays, 'days')) : moment(formValues.ljsj_js).isAfter(moment(formValues.ljsj_ks).add(exportListDataMaxDays, 'days'));
  ...
  changeToListPage = (search, dateArry) => {
    ...
    this.props.form.setFieldsValue({
      [search && search.timeName ? search.timeName : 'rksj']: [moment(dateArry[0], 'YYYY-MM-DD'), moment(dateArry[1], 'YYYY-MM-DD')],
...
// 展开筛选和关闭筛选
  ...
  render() {
    ...
    const caseProcessDictGroup = caseProcessDict.length > 0 ? caseProcessDict.map(item => {
      return <Option key={item.code} value={item.code}>{item.name}</Option>;
    }) : null;
