        ...
        this.props.dispatch({
            ...
            callback: (data) => {
                if (data.list[0].fkzt !== '1') {
                    ...
                    this.getDossierDetail(this.props.location.query.id, this.props.location.query.record.system_id);
                }
            },
        });
    };
                      ...
                      <span style={{ float: 'right', margin: '12px 16px 12px 0',height:32 }}>
                        {DossierDetailData && DossierDetailData.zt === '待督办' && isDb ?
                            <Button type="primary" style={{ marginLeft: 8 }} loading={this.state.loading1}
                                    onClick={() => this.onceSupervise(true, DossierDetailData)}>督办</Button>
                            :
                            ''
                        }
                          {
                              DossierDetailData && (DossierDetailData.dbid === '' || (DossierDetailData.dbList && DossierDetailData.dbList.length > 0 && DossierDetailData.dbList[DossierDetailData.dbList.length - 1].fkzt !== '1')) && isDb ? (
                                  <Button
                                      type="primary"
                                      style={{ marginLeft: 8 }}
            ...
            stap1.push(
                ...
                >
                    ...
                    <Row style={{ paddingLeft: 30, paddingBottom: 8 }}>
                        ...
                        {
                            relevanceInfo[relevanceInfo.length - i - 1].dossier_current_custody_details && relevanceInfo[relevanceInfo.length - i - 1].dossier_current_custody_details.cabinet_id ?
                                relevanceInfo[relevanceInfo.length - i - 1].dossier_current_custody_details.cabinet_id === '106202002' ?
                                    <Col md={7}
                ...
                <Card title="| 卷宗信息" className={styles.wpxxcard} bordered={false}>
                    <Row gutter={rowLayout}>
                        <Col {...colLayout}>
                            ...
                            <div className={styles.Indextail} style={{ paddingLeft: 86 }}>{DossierDetailData && DossierDetailData.ajxxList && DossierDetailData.ajxxList.length > 0 && DossierDetailData.ajxxList[0].ajmc ? DossierDetailData.ajxxList[0].ajmc : (DossierDetailData.ajmc ? DossierDetailData.ajmc : '')}</div>
                        </Col>
                        <Col {...colLayout}>
                            <div className={styles.Indexfrom}>案件编号：</div>
                            <div className={styles.Indextail} style={{ paddingLeft: 86 }}>{DossierDetailData && DossierDetailData.ajxxList.length > 0 && DossierDetailData.ajxxList[0].ajbh ? DossierDetailData.ajxxList[0].ajbh : (DossierDetailData.ajbh ? DossierDetailData.ajbh : '')}</div>
                        </Col>
                        <Col {...colLayout}>
                            <div className={styles.Indexfrom}>案件类型：</div>
                            <div className={styles.Indextail} style={{ paddingLeft: 86 }}>{DossierDetailData && DossierDetailData.ajxxList.length > 0 && DossierDetailData.ajxxList[0].ajlx_mc ? DossierDetailData.ajxxList[0].ajlx_mc : (DossierDetailData.ajlx_mc ? DossierDetailData.ajlx_mc : '')}</div>
                        </Col>
                    </Row>
                    <Row gutter={rowLayout}>
                        <Col {...colLayout}>
                            <div className={styles.Indexfrom}>案件状态：</div>
                            <div className={styles.Indextail} style={{ paddingLeft: 86 }}>{DossierDetailData && DossierDetailData.ajxxList.length > 0 && DossierDetailData.ajxxList[0].ajzt ? DossierDetailData.ajxxList[0].ajzt : (DossierDetailData.ajzt ? DossierDetailData.ajzt : '')}</div>
                        </Col>
                        <Col {...colLayout}>
                            <div className={styles.Indexfrom}>办案单位：</div>
                            <div className={styles.Indextail} style={{ paddingLeft: 86 }}>{DossierDetailData && DossierDetailData.ajxxList.length > 0 && DossierDetailData.ajxxList[0].bardwmc ? DossierDetailData.ajxxList[0].bardwmc : (DossierDetailData.bardw_name ? DossierDetailData.bardw_name : '')}</div>
                        </Col>
                        <Col {...colLayout}>
                            <div className={styles.Indexfrom}>办案人：</div>
                            <div className={styles.Indextail} style={{ paddingLeft: 74 }}>{DossierDetailData && DossierDetailData.ajxxList.length > 0 && DossierDetailData.ajxxList[0].barxm ? DossierDetailData.ajxxList[0].barxm : (DossierDetailData.bar ? DossierDetailData.bar : '')}</div>
                        </Col>
                    </Row>
                    <Row gutter={rowLayout}>
                        <Col {...colLayout}>
                            <div className={styles.Indexfrom}>卷宗名称：</div>
                            <div className={styles.Indextail} style={{ paddingLeft: 86 }}>{DossierDetailData && DossierDetailData.jzmc ? DossierDetailData.jzmc : ''}</div>
                        </Col>
                        <Col {...colLayout}>
                            <div className={styles.Indexfrom}>卷宗编号：</div>
                            <div className={styles.Indextail} style={{ paddingLeft: 86 }}>{DossierDetailData && DossierDetailData.jzbh ? DossierDetailData.jzbh : ''}</div>
                        </Col>
                        <Col {...colLayout}>
                            <div className={styles.Indexfrom}>立卷人：</div>
                            <div className={styles.Indextail} style={{ paddingLeft: 74 }}>{DossierDetailData && DossierDetailData.ljr ? DossierDetailData.ljr : ''}</div>
                        </Col>
                    </Row>
                    <Row gutter={rowLayout}>
                        <Col {...colLayout}>
                            <div className={styles.Indexfrom}>卷宗类别：</div>
                            <div className={styles.Indextail} style={{ paddingLeft: 86 }}>{DossierDetailData && DossierDetailData.jzlb_mc ? DossierDetailData.jzlb_mc : ''}</div>
                        </Col>
                        <Col {...colLayout}>
                            <div className={styles.Indexfrom}>卷宗页数：</div>
                            <div className={styles.Indextail} style={{ paddingLeft: 86 }}>{DossierDetailData && DossierDetailData.jzys ? DossierDetailData.jzys : ''}</div>
                        </Col>
                        <Col {...colLayout}>
                            <div className={styles.Indexfrom}>立卷时间：</div>
                            <div className={styles.Indextail} style={{ paddingLeft: 86 }}>{DossierDetailData && DossierDetailData.ljsj ? DossierDetailData.ljsj : ''}</div>
                        </Col>
                    </Row>
                    <Row gutter={rowLayout}>
                        <Col {...specialcolLayout} className={liststyles.JzInfoRight}>
                            <div className={styles.Indexfrom}>卷宗描述：</div>
                            <div className={styles.Indextail} style={{ paddingLeft: 86 }}>{DossierDetailData && DossierDetailData.jzms ? DossierDetailData.jzms : ''}</div>
                        </Col>
                    </Row>
                </Card>
