...
export default class PersonalDoc extends PureComponent {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      ...
      nextProps.global.isResetList.url === '/lawEnforcement/PersonFile'
    ) {
  ...
  getAllList = props => {
    if (
      ...
      this.setState({
        ...
        salx:
          props.location.state.params && props.location.state.params.name
            ? props.location.state.params.name
            : '',
        searchHeight: true,
      });
      const params = {
      ...
      const qzcsfxqssj = searchTime
        ? moment(searchTime)
            .startOf('month')
            .format('YYYY-MM-DD')
        : '';
      const qzcsfxzzsj = searchTime
        ? moment(searchTime)
            .endOf('month')
            .format('YYYY-MM-DD')
        : '';
  ...
  exportData = () => {
    ...
    if (
      (time && time.length > 0) ||
      (fxtime && fxtime.length > 0) ||
      (sltime && sltime.length > 0)
    ) {
      const isAfterDate = moment(formValues.cjrq_js).isAfter(
        moment(formValues.cjrq_ks).add(exportListDataMaxDays, 'days'),
      );
      const isAfterDate1 = moment(formValues.qzcsfxqssj).isAfter(
        moment(formValues.qzcsfxzzsj).add(exportListDataMaxDays, 'days'),
      );
      const isAfterDate2 = moment(formValues.slqssj).isAfter(
        moment(formValues.slzzsj).add(exportListDataMaxDays, 'days'),
      );
      if (isAfterDate || isAfterDate1 || isAfterDate2) {
  ...
  renderForm() {
    ...
    const {
      ...
      common: { depTree, involvedType, enforcementTypeDict },
    } = this.props;
    ...
    const caseTypeTree = [];
    caseTypeTreeXZ.map(item => caseTypeTree.push(item));
    caseTypeTreeXS.map(item => caseTypeTree.push(item));
  ...
  render() {
    ...
    return (
      <div className={stylescommon.statistics}>
        ...
        {this.renderTable()}
        <SyncTime
          dataLatestTime={personData.tbCount ? personData.tbCount.tbsj : ''}
          {...this.props}
        />
      </div>
    );
