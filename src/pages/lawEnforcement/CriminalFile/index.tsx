...
export default class CriminalFile extends PureComponent {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      ...
      nextProps.global.isResetList.url === '/lawEnforcement/File/CriminalFile'
    ) {
      this.setState({
  ...
  exportData = () => {
    ...
    if (
      (newformValues.jarq_ks && newformValues.jarq_js) ||
      (newformValues.xarq_ks && newformValues.xarq_js) ||
      (newformValues.parq_ks && newformValues.parq_js) ||
      (newformValues.sarq_ks && newformValues.sarq_js) ||
      (newformValues.larq_ks && newformValues.larq_js) ||
      (newformValues.qsrq_ks && newformValues.qsrq_js)
    ) {
      const saisAfterDate =
        newformValues.sarq_js && newformValues.sarq_ks
          ? moment(newformValues.sarq_js).isAfter(
              moment(newformValues.sarq_ks).add(exportListDataMaxDays, 'days'),
            )
          : true;
      const laisAfterDate =
        newformValues.larq_js && newformValues.larq_ks
          ? moment(newformValues.larq_js).isAfter(
              moment(newformValues.larq_ks).add(exportListDataMaxDays, 'days'),
            )
          : true;
      const paisAfterDate =
        newformValues.parq_js && newformValues.parq_ks
          ? moment(newformValues.parq_js).isAfter(
              moment(newformValues.parq_ks).add(exportListDataMaxDays, 'days'),
            )
          : true;
      const xaisAfterDate =
        newformValues.xarq_js && newformValues.xarq_ks
          ? moment(newformValues.xarq_js).isAfter(
              moment(newformValues.xarq_ks).add(exportListDataMaxDays, 'days'),
            )
          : true;
      const jaisAfterDate =
        newformValues.jarq_js && newformValues.jarq_ks
          ? moment(newformValues.jarq_js).isAfter(
              moment(newformValues.jarq_ks).add(exportListDataMaxDays, 'days'),
            )
          : true;
      const isAfterDate2 =
        newformValues.qsrq_js && newformValues.qsrq_ks
          ? moment(newformValues.qsrq_js).isAfter(
              moment(newformValues.qsrq_ks).add(exportListDataMaxDays, 'days'),
            )
          : true;
      if (
        saisAfterDate &&
  ...
  render() {
    ...
    return (
      <div
        className={dark ? stylescommon.statistics : styles.ligthBox + ' ' + stylescommon.statistics}
      >
        {window.configUrl.is_area === '1' ? (
