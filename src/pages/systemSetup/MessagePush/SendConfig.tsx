...
class SendConfig extends Component {
  ...
  getTimeChange = nextProps => {
    ...
    let time =
      messageList && messageList.mdrsd ? messageList.mdrsd.replace('$$1', '').split(',') : '';
    let startTime = [
      time && time[0] ? time[0].split('~')[0] : '',
      time && time[1] ? time[1].split('~')[0] : '',
      time && time[2] ? time[2].split('~')[0] : '',
    ];
    let endTime = [
      time && time[0] ? time[0].split('~')[1] : '',
      time && time[1] ? time[1].split('~')[1] : '',
      time && time[2] ? time[2].split('~')[1] : '',
    ];
  ...
  timeList = () => {
    ...
    const {
      systemSetup: { messageList },
    } = this.props;
    let time =
      messageList && messageList.mdrsd ? messageList.mdrsd.replace('$$1', '').split(',') : null;
    for (let i = 0; i < 3; i++) {
      ...
      timeList.push(
        <div>
          <TimePicker
            value={
              this.state['startTime' + i]
                ? this.state['startTime' + i] === 'clear'
                  ? null
                  : this.state['startTime' + i]
                : startTime
          ...
          />{' '}
          <span style={{ color: dark ? '#fff' : '#999' }}> ~ </span>
          <TimePicker
            value={
              this.state['endTime' + i]
                ? this.state['endTime' + i] === 'clear'
                  ? null
                  : this.state['endTime' + i]
                : endTime
            ...
            defaultOpenValue={moment('00:00:00', 'HH:mm:ss')}
          />
          <span onClick={this.getAdd} className={i == 0 ? style.iconAdd : style.none}>
            <span> {this.state.autoHeight ? '折叠' : '展开'} </span>
            <Icon type={this.state.autoHeight ? 'up' : 'down'} />
          </span>
        </div>,
      );
    }
  ...
  render() {
    ...
    return (
      <div id={'boxSend'}>
          ...
          <Form>
            ...
            <Form.Item label="免打扰时段" {...formItemLayout}>
              {getFieldDecorator('mdrsd', {
                ...
                <div
                  style={{ height: this.state.autoHeight ? 'auto' : '40px', overflow: 'hidden' }}
                >
                  {this.timeList()}
                </div>,
              )}
        ...
        <Card>
          <Button type="primary" className={style.submitButton} onClick={this.showConfirm}>
            提交
          </Button>
        </Card>
      </div>
