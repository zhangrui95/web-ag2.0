...
class SuperviseSetup extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      this.props.SuperviseSetup.global.isResetList.isReset !==
        nextProps.SuperviseSetup.global.isResetList.isReset &&
      nextProps.SuperviseSetup.global.isResetList.url === '/systemSetup/SuperviseSetup'
    ) {
      this.getJgdList(
        this.state.pd,
        nextProps.history.location.query.type === '0' ? 1 : this.state.current,
      );
    }
  }
    ...
    this.props.form.validateFields((err, values) => {
      let pd = {
        jgd_dm:
          values.jgd &&
          values.jgd.key &&
          JSON.parse(values.jgd.key) &&
          JSON.parse(values.jgd.key).code
            ? JSON.parse(values.jgd.key).code
            : '',
  ...
  addList = (type, reson) => {
    ...
    this.props.dispatch(
      routerRedux.push({
        pathname:
          type === 0
            ? '/systemSetup/SuperviseSetup/Add'
            : type === 2
            ? '/systemSetup/SuperviseSetup/Update'
            : '/systemSetup/SuperviseSetup/Detail',
    ...
    const columns = [
      {
        ...
        render: (text, record) => {
          return text ? (
            <div>
              <span>{text}</span>
              <Tooltip title={record.lxsm}>
                <Icon type="info-circle" className={styles.lxsm} />
              </Tooltip>
            </div>
          ) : (
            ''
      ...
      },
      {
        ...
        render: (text, record) => {
          return record.jgqx_ks && record.jgqx_js ? (
            <div>
      ...
      },
      {
        ...
        render: record => (
          ...
          >
            {record.scbj === '0' ? record.jgdzt_mc + '（未启用）' : record.jgdzt_mc}
          </Tag>
        ),
      },
      {
                  ...
                  {getFieldDecorator('jgd', {
                    ...
                    >
                      ...
                      {JgdType &&
                        JgdType.map(event => {
                          return (
                            <Option value={JSON.stringify({ code: event.code, name: event.name })}>
                              {event.name}
                            </Option>
                          );
                        })}
        ...
        <Card className={stylescommon.cardArea} id={'box'}>
          <Table
            ...
            locale={{
              emptyText: (
                <Empty
                  image={this.props.SuperviseSetup.global.dark ? noList : noListLight}
                  description={'暂无数据'}
                />
              ),
            }}
