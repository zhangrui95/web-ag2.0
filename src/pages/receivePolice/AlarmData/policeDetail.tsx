            ...
            if (this.state.IsSure) {
                this.props.dispatch({
                    ...
                    payload: {
                        ...
                        wtid: res.wtid,
                        ajbh: res.ajbh,
                        system_id: this.props.tzlx === 'jqyj'||this.props.tzlx === 'jqxx' ? policeDetails.id : policeDetails.system_id,
                        ajGzLx: ajGzLx,
                    },
                    callback: (res) => {
                        if (!res.error) {
        ...
        let detail = (
            <Row style={{ width: '90%', margin: '0 38px 10px', lineHeight: '36px', color: 'rgba(0, 0, 0, 0.85)' }}>
                ...
                <Col span={9}>管辖单位：<Tooltip
                    title={policeDetails && policeDetails.jjdw && policeDetails.jjdw.length > 13 ? policeDetails.jjdw : null}>{policeDetails && policeDetails.jjdw ? policeDetails.jjdw.length > 13 ? policeDetails.jjdw.substring(0, 13) + '...' : policeDetails.jjdw : ''}</Tooltip></Col>
                <Col span={10}>接警信息：<LeightWord keyWord={this.state.keyWord ? this.state.keyWord : []}
                                                newsString={policeDetails && policeDetails.jjnr ? policeDetails.jjnr : ''}/></Col>
                <Col span={5}>处警人：{policeDetails && policeDetails.cjr ? policeDetails.cjr : ''}</Col>
                <Col span={9}>处警单位：<Tooltip
                    title={policeDetails && policeDetails.cjdw && policeDetails.cjdw.length > 13 ? policeDetails.cjdw : null}>{policeDetails && policeDetails.cjdw ? policeDetails.cjdw.length > 13 ? policeDetails.cjdw.substring(0, 13) + '...' : policeDetails.cjdw : ''}</Tooltip></Col>
                <Col span={10}>处警信息：<Tooltip
                    title={policeDetails && policeDetails.cjqk && policeDetails.cjqk.length > 16 ? policeDetails.cjqk : null}>{policeDetails && policeDetails.cjqk ? policeDetails.cjqk.length > 16 ? policeDetails.cjqk.substring(0, 16) + '...' : policeDetails.cjqk : ''}</Tooltip></Col>
