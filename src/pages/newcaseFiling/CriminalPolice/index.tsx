/*
 * CriminalPolice/index.tsx 合并后受立案刑事案件告警数据
 * author：jhm
 * 20180605
 * */
...
import UnCaseDataView from '../../../components/UnCaseRealData/UnCaseEnforcementDataViewAll';
...
@connect(({ UnCaseData, loading, common, MySuperviseData,global }) => ({
  UnCaseData, loading, common, MySuperviseData,global
...
// 获取案件状态字典
  ...
  renderForm() {
    ...
    return (
      <Form onSubmit={this.handleSearch} style={{ height: this.state.searchHeight ? 'auto' : '50px' }}>
          ...
          <Col {...colLayout}>
            <FormItem label="问题类型" {...formItemLayout}>
              {getFieldDecorator('wtlx', {
                ...
                <Select placeholder="请选择问题类型" style={{ width: '100%' }} getPopupContainer={() => document.getElementById('newslaxsajgjtableListForm')}>
            ...
            <FormItem label="消息状态" {...formItemLayout}>
              {getFieldDecorator('dbzt', {
                ...
                <MessageState superviseStatusOptions={superviseStatusOptions}
                              ...
                              newId='newslaxsajgjtableListForm' />,
              )}
            </FormItem>
          </Col>
        </Row>
        <Row gutter={rowLayout} className={styles.searchForm}>
          <Col {...colLayout}>
            <FormItem label="案件状态" {...formItemLayout}>
              {getFieldDecorator('ajzt', {
                ...
                <Select placeholder="请选择案件状态" style={{ width: '100%' }} getPopupContainer={() => document.getElementById('newslaxsajgjtableListForm')}>
                  <Option value="">全部</Option>
                  {CaseStatusOption}
                </Select>,
              )}
            ...
            <FormItem label="告警时间" {...formItemLayout}>
              {getFieldDecorator('gjsj', {
                ...
                <RangePicker
                  ...
                  style={{ width: '100%' }}
                  getCalendarContainer={() => document.getElementById('newslaxsajgjtableListForm')}
                />,
              )}
            </FormItem>
          </Col>
          <Col {...colLayout}>
            <FormItem label="受案单位" {...formItemLayout}>
              {getFieldDecorator('sadw', {
                initialValue: this.state.sadw ? this.state.sadw : undefined,
              })(
                <TreeSelect
                  showSearch
                  style={{ width: '100%' }}
          ...
          <Col {...colLayout}>
            <FormItem label="产生方式" {...formItemLayout}>
              {getFieldDecorator('csfs', {
                ...
                <Select placeholder="请选择产生方式" style={{ width: '100%' }}  onChange={this.getCsfs} getPopupContainer={() => document.getElementById('newslaxsajgjtableListForm')}>
                  <Option value="">全部</Option>
        ...
        <Row className={styles.search} style={{ position: 'absolute', top: 16, right: 32 }}>
  ...
  renderTable() {
    const { UnCaseData: { unCaseData, loading } } = this.props;
    return (
      <div>
        <RenderTable
          // loading={loading}
      ...
      <div className={this.props.location.query && this.props.location.query.id ? styles.onlyDetail : ''}>
            <div className={className}>
              ...
              <div style={showDataView ? { display: 'none' } : { display: 'block' }}>
                <div className={styles.tableListForm} style={{position:'relative'}} id="newslaxsajgjtableListForm">
                  {this.renderForm()}
                </div>
