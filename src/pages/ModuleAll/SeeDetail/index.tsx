...
export default class DbHistory extends PureComponent {
    ...
    title(Isdetail) {
        if (Isdetail === '发起督办') {
            return (
                <div style={{color: '#fff'}}>督办详情</div>
            );
        } else {
            return (
                <div style={{color: '#fff'}}>反馈详情</div>
            );
        }
                ...
                <Card className={styles.standardTable} id='SeeDetailModule'>
                    <div>
                        <Row gutter={{md: 8, lg: 24, xl: 48}}>
                            {Isdetail === '发起督办' ? '' : <Col md={8} sm={24}>
                                <div className={styles.dbrz}>{record.zrrMcOfXq ? record.zrrMcOfXq : ''}</div>
                            </Col>}
                            {Isdetail === '发起督办' ? '' : <Col md={16} sm={24}>
                                <div className={styles.dbrz}>{record.zrdwMcOfXq ? record.zrdwMcOfXq : ''}</div>
                            </Col>}
                            <Col md={8} sm={24}>
                                <div
                                    className={styles.dbrz}>{Isdetail === '发起督办' ? record.zrrMc : record.fkrXmOfXq ? record.fkrXmOfXq : ''}</div>
                            </Col>
                            <Col md={Isdetail === '发起督办' ? 8 : 16} sm={24}>
                                <div
                                    className={styles.dbrz}>{Isdetail === '发起督办' ? record.zrdwMc : record.fkrdwOfXq ? record.fkrdwOfXq : ''}</div>
                            </Col>
                            <Col md={8} sm={24}>
                                <div
                                    className={styles.dbrz}>{Isdetail === '发起督办' ? record.clsj ? record.clsj : '' : record.fksj ? record.fksj : ''}</div>
                            </Col>
                            <Col md={8} sm={24}>
                                <div className={styles.historydbrz}>问题类型：<span
                                    className={styles.dbrzName}>{record.wtlxMc}</span></div>
                            </Col>
                            {Isdetail === '发起督办' ? '' :
                                <Col md={8} sm={24}>
                                    <div className={styles.historydbrz}><span
                                        className={styles.dbrzName}>{record.cljg ? record.cljg : ''}</span></div>
                                </Col>}
                        </Row>
                        <Row gutter={{md: 8, lg: 24, xl: 48}}>
                            <Col md={24} sm={24}>
                            ...
                            <Col md={24} sm={24}>
                                <div className={styles.historydbrz}>
                                    <span
                                        className={styles.dbrzName1}>{Isdetail === '发起督办' ? '整改意见：' : ''}{record.zgyj}</span>
                                </div>
                            </Col>
                        </Row>
