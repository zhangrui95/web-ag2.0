...
export default class DbHistory extends PureComponent {
        ...
        <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
          <Col md={24} sm={24}>
            <div className={styles.historydbrz}>
              问题类型：<span className={styles.dbrzName}>{pane.wtlxMc}</span>
            </div>
          </Col>
        </Row>
        {this.stepname(pane)}
  ...
  title() {
    return <div style={{ color: '#fff' }}>历史督办日志</div>;
  }
        ...
        <Card className={styles.standardTable} id="DbhistoryModule">
          <div>
            <div className={styles.title}>
              <Steps
                direction="vertical"
                current={newDblist.length > 0 ? newDblist.length - 1 : ''}
              >
                {newDblist
                  ? newDblist.map(pane => (
                      <Step
                        title={<span style={{ fontSize: 14 }}>{pane.dbztMc}</span>}
