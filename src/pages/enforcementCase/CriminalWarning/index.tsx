...
export default class Index extends PureComponent {
  ...
  saveShare = (res, type, ajGzLx) => {
    this.setState({
      sx: (res.ajmc ? res.ajmc + '、' : '') + (res.yjlxmc ? res.yjlxmc + '、' : '') + (res.yjsj ? res.yjsj : ''),
      shareRecord: res,
    });
    if (type === 3) {
      this.setState({
...
// 展开筛选和关闭筛选
  ...
  render() {
    const { form: { getFieldDecorator }, common: { depTree, superviseStatusDict, YJJBType, XsyjType }, EarlyWarning: { data: { page, list, tbCount } }, loading } = this.props;
                  ...
                  <Col {...colLayout}>
                    <FormItem label="预警类型" {...formItemLayout}>
                      {getFieldDecorator('yjlx', {
                        ...
                        <Select placeholder="请选择预警类型" style={{ width: '100%' }} getPopupContainer={() => document.getElementById('zfbaxsajyjtableListForm')}>
                  ...
                  <Col {...colLayout}>
                    <FormItem label="预警级别" {...formItemLayout}>
                      {getFieldDecorator('yjjb', {
                        ...
                        <Select placeholder="请选择预警级别" style={{ width: '100%' }} getPopupContainer={() => document.getElementById('zfbaxsajyjtableListForm')}>
                          <Option value="">全部</Option>
                          {YJJBStatusOptions}
                        </Select>,
                      )}
                    ...
                    <FormItem label="预警时间" {...formItemLayout}>
                      {getFieldDecorator('yjsj')(
                        <RangePicker
                          ...
                          style={{ width: '100%' }}
                          getCalendarContainer={() => document.getElementById('zfbaxsajyjtableListForm')}
                        />,
                      )}
                    </FormItem>
                  </Col>
