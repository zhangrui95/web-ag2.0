...
@connect(({ UnareaData, loading, common, MySuperviseData }) => ({
  UnareaData, loading, common, MySuperviseData,
...
// 获取办案区树
  ...
  // 渲染办案区树
  renderBaqloop = data => data.map((item) => {
    if (item.children && item.children.length) {
      return <TreeNode value={item.name} key={item.id} title={item.name}
                       selectable={item.code === 'null'}>{this.renderBaqloop(item.children)}</TreeNode>;
    }
    return <TreeNode key={item.id} value={item.name} title={item.name} selectable={item.code === 'null'}/>;
  });
  // 打开督办模态框
  ...
  renderForm() {
    const { form: { getFieldDecorator }, common: { WtlxBaqTypeData, superviseStatusDict, depTree, involvedType, baqTree, rectificationStatusDict } } = this.props;
    ...
    let problemTypeOptions = [], superviseStatusOptions = [], involvedTypeOptions = [];
    ...
    return (
      <Form onSubmit={this.handleSearch} style={{height: this.state.searchHeight ? 'auto' : '59px',position:'relative'}}>
          ...
          <Col {...colLayout}>
            <FormItem label="问题类型" {...formItemLayout}>
              {getFieldDecorator('wtlx', {
                ...
                <Select placeholder="请选择问题类型" style={{ width: '100%' }} getPopupContainer={() => document.getElementById('baqgjsjtableListForm')}>
          ...
          <Col {...colLayout}>
            <FormItem label="人员类型" {...formItemLayout}>
              {getFieldDecorator('salx', {
                ...
                <Select placeholder="请选择人员类型" style={{ width: '100%' }} getPopupContainer={() => document.getElementById('baqgjsjtableListForm')}>
                  <Option value="">全部</Option>
                  {involvedTypeOptions}
                </Select>,
              )}
          ...
          <Col {...colLayout}>
            <FormItem label="产生方式" {...formItemLayout}>
              {getFieldDecorator('csfs', {
                ...
                <Select placeholder="请选择产生方式" style={{ width: '100%' }} onChange={this.getCsfs} getPopupContainer={() => document.getElementById('baqgjsjtableListForm')}>
                  <Option value="">全部</Option>
  ...
  renderTable() {
    const { UnareaData: { Unarea, loading } } = this.props;
    return (
      <div>
        <RenderTable
          {...this.props}
