...
@connect(({ UnareaData, loading, MySuperviseData, areaData, AllDetail }) => ({
    UnareaData, loading, MySuperviseData, areaData, AllDetail,
...
export default class unareaDetail extends PureComponent {
    ...
    componentDidMount() {
      ...
      if (location && location.query && location.query.record && location.query.record.id && location.query.record.baq_id) {
    ...
    onceSupervise = (flag, UnareaDetail) => {
        ...
        const { wtlx, wtid, ryxx: { badw, badwDm, bar, barzjhm } } = UnareaDetail;
        this.props.dispatch({
                    ...
                    <Col>
                        <span style={{ float: 'right', margin: '0 16px 12px 0' }}>
                          {UnareaDetail && UnareaDetail.zt === '待督办' && isDb ?
                              <Button type="primary" style={{ marginLeft: 8 }} className={styles.TopMenu} loading={this.state.loading1}
                                      onClick={() => this.onceSupervise(true, UnareaDetail)}>督办</Button>
                              :
                              ''
                          }
                            {
                                UnareaDetail && (UnareaDetail.dbid === '' || (UnareaDetail.dbList && UnareaDetail.dbList.length > 0 && UnareaDetail.dbList[0].fkzt !== '1')) && isDb ? (
                                    <Button
                                        type="primary"
                                        style={{ marginLeft: 8 }}
...
// 日志左右切换
    ...
    Title = (roomName, trackTime, trackLeftTime, paneData) => {
        return (
            <div className={styles.trajectory} onClick={() => this.trajectoryTitle(paneData)}>
                <p className={styles.clsj_times}><Ellipsis lines={1} tooltip>{roomName}</Ellipsis></p>
    ...
    AllButton1 = (newObjWidth, length) => {
        if (newObjWidth === 1280) {
            if (length > 4) {
                return (
                    <div className={styles.IconStyle} style={{ width: '186px', top: '50%', marginTop: '-30px' }}>
    ...
    trailtitle(UnareaDetail) {
        return (
            <div>
                ...
                <a className={styles.traillink} onClick={() => this.playVideo(UnareaDetail)}>播放完整视频</a>
            </div>
        );
    }
    ...
    renderDetail() {
        ...
        let newObjWidth = '', superveWidth = '', trackWidth = '';
        ...
        const extra = (
            <div className={styles.tableExtra}>
                <div className={styles.track} style={{ width: trackWidth }}>
                    <Steps progressDot className={styles.steps} style={{ left: this.state.trailLeft, top: '70px' }}
                           current={UnareaDetail && UnareaDetail.trackList && UnareaDetail.trackList.length > 0 ? UnareaDetail.trackList.length : 1}>
                        {UnareaDetail && UnareaDetail.trackList && UnareaDetail.trackList.length > 0 ? UnareaDetail.trackList.map(pane =>
                ...
                {UnareaDetail && UnareaDetail.trackList && UnareaDetail.trackList.length > 0 ?
                    <div>
                        {this.AllButton1(newObjWidth, UnareaDetail.trackList.length)}
                    </div>
                    :
                    ''
                }
        ...
        return (
            <div style={{ background: '#252c3c', /*height: autoheight() - 180 + 'px'*/ }}
                            ...
                            <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
                                <Col md={5} sm={24}>
                                    ...
                                    <div className={styles.Indextail} style={{ paddingLeft: '42px' }}>
                                        <a style={{ textDecoration: 'underline' }} onClick={() => this.openPersonDetail(UnareaDetail.ryxx.xyr_sfzh, UnareaDetail.ryxx.name)}>{UnareaDetail && UnareaDetail.ryxx && UnareaDetail.ryxx.name ? UnareaDetail.ryxx.name : ''}</a>
                                    </div>
                                </Col>
                                <Col md={5} sm={24}>
                                    <div className={styles.Indexfrom}>年龄：</div>
                                    <div className={styles.Indextail} style={{ paddingLeft: '42px' }}>{UnareaDetail && UnareaDetail.ryxx && UnareaDetail.ryxx.age ? UnareaDetail.ryxx.age : ''}</div>
                                </Col>
                                <Col md={4} sm={24}>
                                    <div className={styles.Indexfrom}>性别：</div>
                                    <div className={styles.Indextail} style={{ paddingLeft: '42px' }}>{UnareaDetail && UnareaDetail.ryxx && UnareaDetail.ryxx.sex ? UnareaDetail.ryxx.sex : ''}</div>
                                </Col>
                                <Col md={5} sm={24}>
                                    <div className={styles.Indexfrom}>证件号码：</div>
                                    <div className={styles.Indextail}>{UnareaDetail && UnareaDetail.ryxx && UnareaDetail.ryxx.zjhm ? UnareaDetail.ryxx.zjhm : ''}</div>
                                </Col>
                                <Col md={5} sm={24}>
                                    <div className={styles.Indexfrom}>人员类型：</div>
                                    <div className={styles.Indextail}>{UnareaDetail && UnareaDetail.ryxx && UnareaDetail.ryxx.salx_mc ? UnareaDetail.ryxx.salx_mc : ''}</div>
                                </Col>
                            </Row>
                            <Row>
                                <Col md={24} sm={24}>
                                    <Card title="涉案信息" className={styles.saxxCard}>
                                        <Row gutter={{ md: 8, lg: 16, xl: 24 }}>
                                                ...
                                                <div className={styles.Indextail}>
                                                    {
                                                        UnareaDetail && UnareaDetail.ajxx && UnareaDetail.ajxx.ajbh ? (
                                                            UnareaDetail.ajxx.system_id && UnareaDetail.ajxx.ajlx ? (
                                                                <a onClick={() => this.openCaseDetail(UnareaDetail.ajxx.system_id, UnareaDetail.ajxx.ajlx, UnareaDetail.ajxx.ajbh)} style={{ textDecoration: 'underline' }}>
                                            ...
                                            <Col md={6} sm={24}>
                                                ...
                                                <div
                                                    className={styles.Indextail}>{UnareaDetail && UnareaDetail.ajxx && UnareaDetail.ajxx.ajmc ? UnareaDetail.ajxx.ajmc : ''}</div>
                                            </Col>
                                            <Col md={6} sm={24}>
                                                <div className={styles.Indexfrom}>案件状态：</div>
                                                <div
                                                    className={styles.Indextail}>{UnareaDetail && UnareaDetail.ajxx && UnareaDetail.ajxx.ajzt ? UnareaDetail.ajxx.ajzt : ''}</div>
                                            </Col>
                                            <Col md={6} sm={24}>
                                                <div className={styles.Indexfrom}>案发时段：</div>
                                                <div
                                                    className={styles.Indextail}>{(UnareaDetail && UnareaDetail.ajxx && UnareaDetail.ajxx.fasjsx && UnareaDetail.ajxx.fasjxx ? UnareaDetail.ajxx.fasjsx + '~' + UnareaDetail.ajxx.fasjxx : '')}</div>
                                            </Col>
                                        </Row>
                                        <Row gutter={{ md: 8, lg: 16, xl: 24 }}>
                                            <Col md={6} sm={24}>
                                                ...
                                                <div
                                                    className={styles.Indextail}>{UnareaDetail && UnareaDetail.ajxx && UnareaDetail.ajxx.bardwmc ? UnareaDetail.ajxx.bardwmc : ''}</div>
                                            </Col>
                                            <Col md={6} sm={24}>
                                                <div className={styles.Indexfrom}>办案人：</div>
                                                <div className={styles.Indextail}
                                                     style={{ paddingLeft: '56px' }}>{UnareaDetail && UnareaDetail.ajxx && UnareaDetail.ajxx.barxm ? UnareaDetail.ajxx.barxm : ''}</div>
                                            </Col>
                                            <Col md={12} sm={24}>
                                                <div className={styles.Indexfrom}>案发地点：</div>
                                                <div
                                                    className={styles.Indextail}>{UnareaDetail && UnareaDetail.ajxx && UnareaDetail.ajxx.afdd ? UnareaDetail.ajxx.afdd : ''}</div>
                                            </Col>
                                        </Row>
                                        <Row gutter={{ md: 8, lg: 16, xl: 24 }}>
                                            <Col md={24} sm={24}>
                                                <div className={styles.Indexfrom}>简要案情：</div>
                                                <div className={styles.Indextail}>{UnareaDetail && UnareaDetail.jyaq ? UnareaDetail.jyaq : ''}</div>
                                            </Col>
                                        </Row>
                                    </Card>
                                </Col>
                        ...
                        <div className={styles.message}>
                            <Row style={{ marginLeft: -24 }}>
                                <Col md={6} sm={24}>
                                    ...
                                    <div className={styles.Indextail}
                                         style={{ paddingLeft: '112px' }}>{UnareaDetail && UnareaDetail.rqxx && UnareaDetail.rqxx.length > 0 && UnareaDetail.rqxx[0].haName ? UnareaDetail.rqxx[0].haName : ''}</div>
                                </Col>
                                <Col md={6} sm={24}>
                                    <div className={styles.Indexfrom}>入区时间：</div>
                                    <div className={styles.Indextail}
                                         style={{ paddingLeft: '98px' }}>{UnareaDetail && UnareaDetail.rqxx && UnareaDetail.rqxx.length > 0 && UnareaDetail.rqxx[0].rqsj ? UnareaDetail.rqxx[0].rqsj : ''}</div>
                                </Col>
                                <Col md={6} sm={24}>
                                    <div className={styles.Indexfrom}>离区时间：</div>
                                    <div className={styles.Indextail}
                                         style={{ paddingLeft: '98px' }}>{UnareaDetail && UnareaDetail.rqxx && UnareaDetail.rqxx.length > 0 && UnareaDetail.rqxx[0].leave_time ? UnareaDetail.rqxx[0].leave_time : ''}</div>
                                </Col>
                                <Col md={6} sm={24}>
                                    <div className={styles.Indexfrom}>滞留时长：</div>
                                    <div className={styles.Indextail}
                                         style={{ paddingLeft: '98px' }}>{UnareaDetail && UnareaDetail.rqxx && UnareaDetail.rqxx.length > 0 && UnareaDetail.rqxx[0].detain_time ? UnareaDetail.rqxx[0].detain_time : ''}</div>
                                </Col>
                            </Row>
                            <Row style={{ marginLeft: -24 }}>
                                <Col md={6} sm={24}>
                                    ...
                                    <div className={styles.Indextail}
                                         style={{ paddingLeft: '98px' }}>{UnareaDetail && UnareaDetail.ryxx && UnareaDetail.ryxx.rqyy ? UnareaDetail.ryxx.rqyy : ''}</div>
                                </Col>
                                <Col md={6} sm={24}>
                                    <div className={styles.Indexfrom}>办案民警：</div>
                                    <div className={styles.Indextail}
                                         style={{ paddingLeft: '98px' }}>{UnareaDetail && UnareaDetail.rqxx && UnareaDetail.rqxx.length > 0 && UnareaDetail.rqxx[0].bar ? UnareaDetail.rqxx[0].bar : ''}</div>
                                </Col>
                                <Col md={12} sm={24}>
                                    <div className={styles.Indexfrom}>办案单位：</div>
                                    <div className={styles.Indextail}
                                         style={{ paddingLeft: '98px' }}>{UnareaDetail && UnareaDetail.rqxx && UnareaDetail.rqxx.length > 0 && UnareaDetail.rqxx[0].badw_mc ? UnareaDetail.rqxx[0].badw_mc : ''}</div>
                                </Col>
                            </Row>
                            <Row style={{ marginLeft: -24 }}>
                                <Col md={24} sm={24}>
                                    ...
                                    <div className={styles.Indextail}
                                         style={{ paddingLeft: '98px' }}>{UnareaDetail && UnareaDetail.ryxx && UnareaDetail.ryxx.rqsy ? UnareaDetail.ryxx.rqsy : ''}</div>
                                </Col>
                            </Row>
                            {UnareaDetail && UnareaDetail.trackList && UnareaDetail.trackList.length > 0 ?
                ...
                <div className={styles.title} style={{ marginTop: '16px' }}>| 随身物品信息</div>
                <div className={styles.tablemessage} style={{ marginBottom: '16px', padding: '24px' }}>
                    <Table
                        ...
                        pagination={{
                            pageSize: 3,
                            showTotal: (total, range) => <div style={{ position: 'absolute', left: '-190px', color:'#fff' }}>共 {(Math.ceil(total / 3))} 页， {total} 条记录</div>,
                            onChange: (page) => {
                ...
                <div className={styles.title} style={{ marginTop: '16px' }}>| 涉案物品信息</div>
    ...
    render() {
        const { superviseVisibleModal, history, UnareaDetail, ssWpList, SsWpId, isState, RestDbrz, reformModal, seeDetail, Isdetail, NowDbrz, feedbackVisibleModal } = this.state;
        // const { getFieldDecorator } = this.props.form;
        return (
            <div>
                <div>
