...
export default class Index extends PureComponent {
  ...
  saveShare = (res, type, ajGzLx) => {
    this.setState({
      sx: (res.baqmc ? res.baqmc + '、' : '') + (res.ajmc ? res.ajmc + '、' : '') + (res.yjlxmc ? res.yjlxmc + '、' : '') + (res.yjsj ? res.yjsj : ''),
      shareRecord: res,
    });
    if (type === 3) {
      this.setState({
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        dataIndex: 'wpmc',
        render: (text) => {
          return <Ellipsis length={15} tooltip>{text}</Ellipsis>;
        },
      },
      {
        title: '案件名称',
        dataIndex: 'ajmc',
        render: (text) => {
          return <Ellipsis length={25} tooltip>{text}</Ellipsis>;
        },
      },
      {
        title: '预警内容',
        ...
        render: (record) => (
          <div>
            ...
            {record.sfgz === 0 ? <Dropdown overlay={<Menu>
              ...
              <a href="javascript:;"
                 onClick={() => this.noFollow(record)}>取消{record.ajgzlx && record.ajgzlx === '0' ? '本人员' : '全要素'}关注</a>}
            <Divider type="vertical"/>
    ...
    let detail = (
      <Row style={{ width: '90%', margin: '0 38px 10px', lineHeight: '36px', color: 'rgba(0, 0, 0, 0.85)' }}>
        <Col
          span={6}>人员姓名：{this.state.areaDetails && this.state.areaDetails.name ? this.state.areaDetails.name : ''}</Col>
        <Col
          span={6}>性别：{this.state.areaDetails && this.state.areaDetails.sex ? this.state.areaDetails.sex : ''}</Col>
        <Col
          span={6}>人员类型：{this.state.areaDetails && this.state.areaDetails.salx_mc ? this.state.areaDetails.salx_mc : ''}</Col>
        <Col span={6}>强制措施：<Tooltip
          title={this.state.areaDetails && this.state.areaDetails.qzcs && this.state.areaDetails.qzcs.length > 7 ? this.state.areaDetails.qzcs : null}>{this.state.areaDetails && this.state.areaDetails.qzcs ? this.state.areaDetails.qzcs.length > 7 ? this.state.areaDetails.qzcs.substring(0, 7) + '...' : this.state.areaDetails.qzcs : ''}</Tooltip></Col>
        <Col span={6}>案件名称：<Tooltip
          title={this.state.areaDetails && this.state.areaDetails.ajmc && this.state.areaDetails.ajmc.length > 7 ? this.state.areaDetails.ajmc : null}>{this.state.areaDetails && this.state.areaDetails.ajmc ? this.state.areaDetails.ajmc.length > 7 ? this.state.areaDetails.ajmc.substring(0, 7) + '...' : this.state.areaDetails.ajmc : ''}</Tooltip></Col>
        <Col span={6}>办案单位：<Tooltip
          title={this.state.areaDetails && this.state.areaDetails.badw && this.state.areaDetails.badw.length > 7 ? this.state.areaDetails.badw : null}>{this.state.areaDetails && this.state.areaDetails.badw ? this.state.areaDetails.badw.length > 7 ? this.state.areaDetails.badw.substring(0, 7) + '...' : this.state.areaDetails.badw : ''}</Tooltip></Col>
        <Col
          span={12}>办案民警：{this.state.areaDetails && this.state.areaDetails.bar ? this.state.areaDetails.bar : ''}</Col>
      </Row>
    );
