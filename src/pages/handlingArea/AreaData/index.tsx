...
// 获取办案区树
  ...
  exportData = () => {
    ...
    if (rqsj && rqsj.length > 0) {
      const isAfterDate = moment(formValues.rqsj_js).isAfter(moment(formValues.rqsj_ks).add(exportListDataMaxDays, 'days'));
  ...
  renderForm() {
    const { form: { getFieldDecorator }, common: { involvedType, depTree, baqTree, rqyyType } } = this.props;
          ...
          <Col {...colLayout}>
            <FormItem label="人员类型" {...formItemLayout}>
              {getFieldDecorator('salx', {
                ...
                <Select placeholder="请选择" style={{ width: '100%' }} getPopupContainer={() => document.getElementById('baqsjtableListForm')}>
  ...
  renderTable() {
    const { areaData: { area, loading } } = this.props;
    return (
      <div>
        <RenderTable
          loading={loading}
              ...
              <div className={styles.listPageHeader}>
                ...
                <span>|</span>
                {
                  showDataView ? (
                    ...
                    <a className={styles.listPageHeaderCurrent}><span>●</span>办案区列表</a>
                  )
                }
                {
                  showDataView?
