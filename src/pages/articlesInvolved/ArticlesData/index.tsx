...
import { Row, Col, Form, Select, message, Input, Button, DatePicker, Tabs, TreeSelect, Icon } from 'antd';
import styles from '../../common/listPage.less';
import RenderTable from '../../../components/ItemRealData/RenderTable';
import { exportListDataMaxDays, getTimeDistance, getUserInfos, tableList } from '../../../utils/utils';
...
@connect(({ itemData, loading, common }) => ({
  itemData, loading, common,
...
export default class Index extends PureComponent {
  ...
  exportData = () => {
    ...
    if ((rkTime && rkTime.length > 0) || (djTime && djTime.length > 0)) {
      const isAfterDate = rkTime && rkTime.length > 0 ? moment(formValues.rksj_js).isAfter(moment(formValues.rksj_ks).add(exportListDataMaxDays, 'days')) : moment(formValues.djrq_js).isAfter(moment(formValues.djrq_ks).add(exportListDataMaxDays, 'days'));
...
// 展开筛选和关闭筛选
  ...
  renderForm() {
    const { form: { getFieldDecorator }, common: { itemsTypesDict, depTree, itemsStorage, itemStatusS, itemsTypesDictNew } } = this.props;
    let itemsTypesOptions = [], itemsStorageOptions = [], itemStatusOption = [];
    ...
    return (
      <Form onSubmit={this.itemSearch} style={{height: this.state.searchHeight ? 'auto' : '59px',}}>
        <Row gutter={rowLayout} className={styles.searchForm}>
          ...
          {
            this.state.timeName && this.state.timeName === 'djsj' ? <Col {...colLayout}>
  ...
  renderTable() {
    const { itemData: { item, loading } } = this.props;
    return (
      <div>
        <RenderTable
          loading={loading}
  ...
  changeToListPage = (name, dateArry) => {
    ...
    this.props.form.setFieldsValue({
      [name && name.timeName ? name.timeName : 'rksj']: [moment(dateArry[0], 'YYYY-MM-DD'), moment(dateArry[1], 'YYYY-MM-DD')],
  ...
  render() {
    const { itemData: { item, loading }, common: { depTree } } = this.props;
              ...
              <div className={styles.listPageHeader}>
                {
                  showDataView ? (
                    ...
                    <a className={styles.UnlistPageHeaderCurrent} onClick={this.changeListPageHeader}>数据统计</a>
                  )
                }
                <span>|</span>
                {
                  showDataView ? (
                    <a className={styles.UnlistPageHeaderCurrent} onClick={this.changeListPageHeader}>数据列表</a>
                  ) : (
