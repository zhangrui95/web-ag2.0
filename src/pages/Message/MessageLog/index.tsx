...
class MessageLog extends Component {
    state = {
        ...
        logDetail: null, // 日志详情
        formValues: {}, // 查询条件
        userOrgCode: JSON.parse(sessionStorage.getItem('user'))
    ...
    exportData = () => {
        ...
        if (tssjTime && tssjTime.length > 0) {
            const isAfterDate = moment(formValues.tssj_js).isAfter(
                moment(formValues.tssj_ks).add(exportListDataMaxDays, 'days'),
            );
            if (isAfterDate) {
