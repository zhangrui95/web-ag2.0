...
// 获取机构树
    ...
    goLook = (record, read) => {
        if (record) {
            if (read === 0) {
                ...
                this.props.dispatch(routerRedux.push({
                    pathname: record.tzlx === 'wpwt' ? '/CaseItem/UnItem/Transfer/Index'
                        : record.tzlx === 'xzajwt1' ? '/register/alarm/alarmAdministration/Index'
                            : record.tzlx === 'xzajwt2' ? '/Enforcement/alarm/alarmAdministration/Index'
                                : record.tzlx === 'xzajwt3' ? '/newregister/newalarm/newalarmAdministration'
                                    : record.tzlx === 'jqwt' ? '/Reception/unpolice/Transfer/Index'
                                        : record.tzlx === 'xsajwt1' ? '/register/alarm/alarmCriminal/Index'
                                            : record.tzlx === 'xsajwt2' ? '/Enforcement/alarm/alarmCriminal/Index'
                                                : record.tzlx === 'xsajwt3' ? '/newregister/newalarm/newalarmCriminal'
                                                    : record.tzlx === 'baqwt' ? '/HandArea/UnArea/Transfer/Index'
                                                        : record.tzlx === 'jzwt' ? '/Dossier/undossier/Transfer/Index'
                                                            : record.tzlx === 'wpxx' ? '/CaseItem/Item/Transfer/Index'
                                                                : record.tzlx === 'xzajxx1' ? '/register/case/Administration/Index'
                                                                    : record.tzlx === 'xzajxx2' ? '/Enforcement/case/Administration/Index'
                                                                        : record.tzlx === 'xzajxx3' ? '/newregister/newcase/newAdministration'
                                                                            : record.tzlx === 'jqxx' ? '/Reception/police/Transfer/Index'
                                                                                : record.tzlx === 'xsajxx1' ? '/register/case/criminal/Index'
                                                                                    : record.tzlx === 'xsajxx2' ? '/Enforcement/case/criminal/Index'
                                                                                        : record.tzlx === 'xsajxx3' ? '/newregister/newcase/newcriminal'
                                                                                            : record.tzlx === 'baqxx' ? '/HandArea/Area/Transfer/Index'
                                                                                                : record.tzlx === 'jzxx' ? '/Dossier/dossier/Transfer/Index'
                                                                                                    : record.tzlx === 'jqyj' ? '/Reception/PoliceIndex'
                                                                                                        : record.tzlx === 'xzajyj1' ? '/register/warn/warnAdministration'
                                                                                                            : record.tzlx === 'xsajyj1' ? '/register/warn/warnCriminal'
                                                                                                                : record.tzlx === 'xzajyj2' ? '/Enforcement/warn/warnAdministration'
                                                                                                                    : record.tzlx === 'xsajyj2' ? '/Enforcement/warn/warnCriminal'
                                                                                                                        : record.tzlx === 'xzajyj3' ? '/newregister/newwarn/newwarnAdministration'
                                                                                                                          : record.tzlx === 'xsajyj3' ? '/newregister/newwarn/newwarnCriminal'
                                                                                                                            : record.tzlx === 'baqyj' ? '/HandArea/AreaIndex'
                                                                                                                                : record.tzlx === 'wpyj' ? '/CaseItem/ItemIndex'
                                                                                                                                    : record.tzlx === 'jzyj' ? '/Dossier/DossierIndex'
                                                                                                                                        : '',
                    query: {
        ...
        if (type) {
            this.setState({
                ...
                columns: [{
                    ...
                    dataIndex: 'ajmc',
                    key: 'ajmc',
                    render: (text) => (<Tooltip placement="top"
                                                title={text}><span>{text && text.length > 15 ? text.substring(0, 15) + '...' : text}</span></Tooltip>),
                }, {
                    title: '案件编号',
                    dataIndex: 'ajbh',
                    key: 'ajbh',
                    ...
                    render: (text, record) => (
                        <span>
            <a href="javascript:;" onClick={() => this.goLook(record, 1)}>查看</a>
          </span>
                    ),
                }],
            });
    ...
    myNews = (page, pageSize) => {
        this.setState({
            ...
            columns: [{
                ...
                key: 'tags',
                dataIndex: 'tags',
                render: tags => (<Tag style={{ cursor: 'default' }} color={tags === '未读' ? '#ee5655' : '#0c0'}
                                      key={tags}>{tags}</Tag>),
            }, {
                title: '发送时间',
                dataIndex: 'time',
                ...
                render: (text, record) => (
                    <span>
            <a href="javascript:;" onClick={() => this.goLook(record, 0)}>查看</a>
          </span>
                ),
            }],
        });
            ...
            callback: (res) => {
                ...
                res.list.map((item, i) => {
                    list.push({
                        ...
                        wtlxMc: item.wtlxMc,
                        ajbh: item.ajbh,
                        xxjs: `${item.ajbh ? item.ajbh : ''}` + `${item.ajbh && item.ajmc ? '、' : ''}` + `${item.ajmc ? item.ajmc : ''}`,
                    });
                });
                if (this.state.idx === 0) {
                    this.setState({
        ...
        this.props.dispatch({
            ...
            callback: (res) => {
                if (!res.error) {
                    ...
                    if (this.state.tabs === 's2') {
                        this.myShare(this.state.pageNew, true, 'Home/getShareList', this.state.pd ? this.state.pd : null, this.state.tabs);
                    } else if (this.state.tabs === 's1') {
                        this.myShare(this.state.pageNew, true, 'Home/getmyShareList', this.state.pd ? this.state.pd : null, this.state.tabs);
                    } else if (this.state.tabs === 'f2') {
                        this.myFollow(this.state.pageNew, true, 'Home/getHistoryFollowList', this.state.pd ? this.state.pd : null, this.state.tabs);
                    }
                    this.myFollow(1, false);
                }
            },
                ...
                columns: [
                  {
                    ...
                    title: tabs && tabs === 's2' || !tabs && this.state.tabs === 's2' ? '分享人' : '被分享人',
                    dataIndex: 'person',
                    key: 'person',
                    render: (text, record) => (
                        tabs && tabs === 's2' || !tabs && this.state.tabs === 's2' ?
                            <Tooltip placement="top"
                                ...
                                {record.czr_dwmc ?
                                    <span>{record.czr_dwmc ? record.czr_dwmc.length > 15 ? record.czr_dwmc.substring(0, 15) + '...' : record.czr_dwmc : ''}<br/></span> : ''}
                                <span>{record.czr ? record.czr : ''}</span>
                            </Tooltip>
                            : <Tooltip placement="top"
                                       title={(record.czr_dwmc ? record.czr_dwmc : '') + (record.fx_xm ? record.fx_xm : '')}>
                                {record.czr_dwmc ?
                                    <span>{record.czr_dwmc ? record.czr_dwmc.length > 15 ? record.czr_dwmc.substring(0, 15) + '...' : record.czr_dwmc : ''}<br/></span> : ''}
                                <span>{record.fx_xm ? record.fx_xm.length > 15 ? record.fx_xm.substring(0, 15) + '...' : record.fx_xm : ''}</span>
                            </Tooltip>),
                }, {
                    title: '分享事项',
                    dataIndex: 'sx',
                    ...
                    render: (text, record) => (
                        <span>
              <a href="javascript:;" onClick={() => this.shareDetail(record)}>查看</a>
              <Divider type="vertical"/>
                            {record.sfgz === 0 ? <Dropdown overlay={<Menu>
                                    <Menu.Item key="0">
                                        <a onClick={() => this.saveShare(record, 1, 0)}>本{record.lx ? record.lx.substring(0, 2) : '案件'}关注</a>
                                    </Menu.Item>
                                    <Menu.Item key="1">
                                ...
                                <a href="javascript:;"
                                   onClick={() => this.noFollow(record.gzid, 2, record.tzlx, record.ajbh, record.ajgzlx)}>取消{record.ajgzlx && record.ajgzlx === '0' ? record.lx ? '本' + record.lx.substring(0, 2) : '本案件' : '全要素'}关注</a>}
            </span>
                    ),
                }],
            });
        ...
        this.props.dispatch({
            ...
            callback: (res) => {
                if (!res.error) {
                    ...
                    if (type === 3) {
                        this.myFollow(this.state.pageNew, true, 'Home/getFollowList', this.state.pd ? this.state.pd : null);
                    } else if (type === 2) {
                        this.myShare(this.state.pageNew, true, this.state.tabs === 's2' ? 'Home/getShareList' : 'Home/getmyShareList', this.state.pd ? this.state.pd : null, this.state.tabs);
                        this.myFollow(1, false);
                    }
                }
            },
                ...
                columns: [
                  {
                    ...
                    key: 'sx',
                    render: (text) => (<Tooltip placement="top"
                                                title={text}><span>{text && text.length > 30 ? text.substring(0, 30) + '...' : text}</span></Tooltip>),
                }, {
                    title: '关注类型',
                    dataIndex: 'lx',
                    key: 'lx',
                    ...
                    render: (text, record) => (
                        <span>
            <a href="javascript:;" onClick={() => this.goLook(record, 3)}>查看</a>
            <Divider type="vertical"/>
                            {tabs && tabs === 'f2' ? <Dropdown overlay={<Menu>
                                    <Menu.Item key="0">
                                        <a onClick={() => this.saveShare(record, 1, 0)}>本{record.lx ? record.lx.substring(0, 2) : '案件'}关注</a>
                                    </Menu.Item>
                                    <Menu.Item key="1">
                                ...
                                <a href="javascript:;"
                                   onClick={() => this.noFollow(record.id, 3, record.tzlx, record.ajbh, record.ajgzlx)}>取消{record.ajgzlx && record.ajgzlx === '0' ? record.lx ? '本' + record.lx.substring(0, 2) : '本案件' : '全要素'}关注</a>}
          </span>
                    ),
                }],
            });
    ...
    handleSearch = (type) => {
        ...
        if (type === 0) {
            ...
            this.myShare(1, true, this.state.tabs === 's2' ? 'Home/getShareList' : 'Home/getmyShareList', pd, this.state.tabs);
        } else {
            const values = this.props.form.getFieldsValue();
            let pd = {
            ...
            this.myFollow(1, true, this.state.tabs === 'f2' ? 'Home/getHistoryFollowList' : 'Home/getFollowList', pd, this.state.tabs);
        }
    };
    //被分享人
    ...
    render() {
        ...
        const paginationPage = {
            ...
            pageSize: this.state.idx === 2 ? this.state.pageSizeShare : this.state.idx === 3 ? this.state.pageSizeFollow : this.state.pageSize,
            size: 'small',
            ...
            onChange: (e) => {
                ...
                if (this.state.idx === 0) {
                    ...
                    this.myShare(e, true, this.state.tabs === 's2' ? 'Home/getShareList' : 'Home/getmyShareList', this.state.pd ? this.state.pd : null, this.state.tabs);
                } else if (this.state.idx === 3) {
                    this.myFollow(e, true, this.state.tabs === 'f2' ? 'Home/getHistoryFollowList' : 'Home/getFollowList', this.state.pd ? this.state.pd : null, this.state.tabs);
                }
            },
        };
        return (
            <div>
                ...
                <Row gutter={16}>
                    <Col span={8} xxl={{ span: 8 }} xl={{ span: 9 }} lg={{ span: 24 }} className={styles.timeLineBox}
                         style={{ marginTop: '16px' }}>
                        <Statistics/>
                    </Col>
                    <Col span={16} xxl={{ span: 16 }} xl={{ span: 15 }} lg={{ span: 24 }} className={styles.tableBox}
                         style={{ marginTop: '16px' }}>
                        {
                ...
                <MyNews visible={this.state.visible} handleCancel={this.handleCancel} datail={this.state.datail}/>
                <MyShare visibleShare={this.state.visibleShare} shareDatail={this.state.shareDatail}
                         handleCancel={this.handleCancel} tabs={this.state.tabs}
                         goLook={(record, read) => this.goLook(record, read)}/>
                <div
                    className={styles.version}>{window.configUrl.headName}&nbsp;&nbsp;版本号：{window.configUrl.version}</div>
            </div>
        );
    }
}
