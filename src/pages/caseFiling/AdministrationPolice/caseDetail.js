...
import {
    ...
    Table,
    Collapse,
    Divider, Select, Icon, Avatar, List, Tooltip, Input, message, Modal,
} from 'antd';
...
@connect(({ UnXzCaseData, loading, UnCaseData, AllDetail, MySuperviseData }) => ({
    UnXzCaseData, loading, UnCaseData, AllDetail, MySuperviseData,
...
// 问题判定
                    ...
                    <span style={{ float: 'right', margin: '12px 16px 12px 0' }}>
                      {caseDetails && caseDetails.zt === '待督办' && isDb ?
                          <Button className={styles.TopMenu} loading={this.state.loading1}
                                  onClick={() => this.onceSupervise(true, caseDetails)}>督办</Button>
                          :
                          ''
                      }
                        {
                            caseDetails && (caseDetails.dbid === '' || (caseDetails.dbList && caseDetails.dbList.length > 0 && caseDetails.dbList[caseDetails.dbList.length - 1].fkzt !== '1')) && isDb ? (
                                <Button
    ...
    render() {
        const { superviseVisibleModal, reformModal, seeDetail, Isdetail, NowDbrz, history, RestDbrz, feedbackVisibleModal } = this.state;
        return (
            <div>
                <div>
                    {this.Topdetail()}
