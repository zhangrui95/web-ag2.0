...
import { autoheight, getQueryString, userAuthorityCode, userResourceCodeDb } from '../../../utils/utils';
...
@connect(({ XzCaseData, loading, CaseData, AllDetail }) => ({
    XzCaseData, loading, CaseData, AllDetail,
...
export default class caseDetail extends PureComponent {
    ...
    componentDidMount() {
      if((this.props.location&&this.props.location.query&&this.props.location.query.record&&this.props.location.query.record.ajbh) || this.props.systemId){
...
// 问题判定
    ...
    saveShare = (caseDetails, res, type, ajGzLx) => {
        this.setState({
            sx: (caseDetails.ajmc ? caseDetails.ajmc + '、' : '') + (caseDetails.ajzt ? caseDetails.ajzt : ''),
        });
        if (type === 2) {
            this.setState({
                shareVisible: true,
            ...
            <div style={{ backgroundColor: '#252C3C', margin:'16px 0' }}>
                <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
                    <Col md={8} sm={24}>
                      ...
                      {
                        isZb ? <Button type="primary" className={styles.TopMenu} onClick={() => this.makeTable(caseDetails, true)}>制表</Button> : null
                      }
                      {/*{ // 案件状态为移送才能退补*/}
                    ...
                    <Row gutter={rowLayout} className={styles.xqrow}>
                        <Col md={8} sm={24} className={styles.xqcol}>
                            ...
                            <div className={liststyles.Indextail} style={{ paddingLeft: 102 }}>
                                <div className={liststyles.special1}><Ellipsis
                                    lines={1}>{caseDetails && caseDetails.ajmc ? caseDetails.ajmc : ''}</Ellipsis></div>
                            </div>
                        </Col>
                        <Col md={8} sm={24} className={styles.xqcol}>
                        ...
                        <Col md={8} sm={24} className={styles.xqcol}>
                            ...
                            <div
                                className={liststyles.Indextail}>{caseDetails && caseDetails.ajlb_name ? caseDetails.ajlb_name : ''}</div>
                        </Col>
                    </Row>
                    <Row gutter={rowLayout} className={styles.xqrow}>
                        <Col md={8} sm={24} className={styles.xqcol}>
                        ...
                        <Col md={8} sm={24} className={styles.xqcol}>
                            ...
                            <div
                                className={liststyles.Indextail}>{caseDetails && caseDetails.fadxz ? caseDetails.fadxz : ''}</div>
                        </Col>
                        <Col md={8} sm={24} className={styles.xqcol}>
    ...
    render() {
        const { superviseVisibleModal, caseDetails, policevisible, resvisible, areavisible, Dossiervisible, makeTableModalVisible } = this.state;
        const JqColumns = [
            {
                title: '接警来源',
                dataIndex: 'jjly_mc',
        ...
        let detail = (
            <Row style={{ width: '90%', margin: '0 38px 10px', lineHeight: '36px', color: 'rgba(0, 0, 0, 0.85)' }}>
                ...
                <Col span={12}>受理单位：<Tooltip
                    title={caseDetails && caseDetails.sldw_name && caseDetails.sldw_name.length > 20 ? caseDetails.sldw_name : null}>{caseDetails && caseDetails.sldw_name ? caseDetails.sldw_name.length > 20 ? caseDetails.sldw_name.substring(0, 20) + '...' : caseDetails.sldw_name : ''}</Tooltip></Col>
                <Col span={12}>案件状态：{caseDetails && caseDetails.ajzt ? caseDetails.ajzt : ''}</Col>
                <Col span={12}>办案民警：{caseDetails && caseDetails.bar_name ? caseDetails.bar_name : ''}</Col>
            </Row>
        );
        return (
            <div id="caseDetail">
