...
export default class Index extends PureComponent {
  ...
  exportData = () => {
    ...
    if ((larqTime && larqTime.length > 0) || (saTime && saTime.length > 0)) {
      const isAfterDate = moment(formValues.larq_js).isAfter(moment(formValues.larq_ks).add(exportListDataMaxDays, 'days'));
  ...
  changeToListPage = (name, dateArry) => {
    ...
    this.props.form.setFieldsValue({
      [name && name.ajzt && name.ajzt === '101' ? 'slrq' : 'larq']: [moment(dateArry[0], 'YYYY-MM-DD'), moment(dateArry[1], 'YYYY-MM-DD')],
    ...
    return (
      <Form onSubmit={this.handleSearch} style={{height: this.state.searchHeight ? 'auto' : '59px',}}>
        <Row gutter={rowLayout} className={styles.searchForm}>
          ...
          {
            (this.state.linkToAjzt && this.state.linkToAjzt === '101')||(statusDate === '101') ? <Col {...colLayout}>
          ...
          <Col {...colLayout}>
            <FormItem label="案件状态" {...formItemLayout}>
              {getFieldDecorator('ajzt', {
                ...
                <Select placeholder="请选择" style={{ width: '100%' }} onChange={this.chooseStatus} getPopupContainer={() => document.getElementById('xsajtableListForm')}>
          ...
          <Col {...colLayout}>
            <FormItem label="专项类别" {...formItemLayout}>
              {getFieldDecorator('zxlb', {
                ...
                <Select placeholder="请选择" style={{ width: '100%' }} onChange={this.specialCaseOnChange} getPopupContainer={() => document.getElementById('xsajtableListForm')}>
                  <Option value="">全部</Option>
        ...
        <Row className={styles.search}>
          <span style={{ float: 'right', marginBottom: 24, marginTop: 5 }}>
              <Button style={{ borderColor: '#2095FF' }} htmlType="submit">查询</Button>
              <Button
                style={{ marginLeft: 8 }}
