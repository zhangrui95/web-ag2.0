...
import {
    ...
    Timeline,
    Table,
    Divider, Select, Icon, Avatar, List, Tooltip, Input, message, Pagination, Modal,
} from 'antd';
...
@connect(({ CaseData, loading, MySuperviseData, AllDetail }) => ({
    CaseData, loading, MySuperviseData, AllDetail,
...
// 修改改变模态框状态 通过id 获取数据
            ...
            <div style={{ backgroundColor: '#252C3C', margin:'16px 0' }}>
                <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
                    <Col md={8} sm={24}>
                      ...
                      {
                        isDb && caseDetails.zrdwList && caseDetails.zrdwList.length > 0 && caseDetails.ssmk === '2' ? (
                          <Button
                            type="primary"
                      ...
                      {
                        isZb ? <Button type="primary" className={styles.TopMenu}
                      ...
                      { // 案件状态为移送才能退补
                        caseDetails.ajzt&&caseDetails.ajzt === '结案' || !isTb || caseDetails.qsrq === '' || (caseDetails.tbrq2 && caseDetails.tbyy2) ? null : (
                          <Button type="primary" className={styles.TopMenu}
                         ...
                         <span style={{ float: 'right',  margin: '6px 16px 6px 0' }}>
                             {
                                 caseDetails ? (
                                     <span>
                                        ...
                                        {sfgz === 0 ?
                                          <Tooltip title="关注">
                                            <img src={nocollect} width={25} height={25} style={{marginLeft:12}} onClick={() => this.saveShare(caseDetails, record, 1, 0)}/>
                                            <div style={{fontSize:12,textAlign:'center',width:48}}>关注</div>
                                          </Tooltip> :
                                          <Tooltip title="取消关注">
                                            <img src={collect} width={25} height={25} style={{marginLeft:12}} onClick={() => this.noFollow(caseDetails)}/>
                                            <div style={{fontSize:12,textAlign:'center',width:48}}>取消关注</div>
                                          </Tooltip>
                                        }
                                      </span>
                                        <span className={liststyles.collect} onClick={() => this.saveShare(caseDetails, record, 2)}>
                                          <Tooltip title="分享">
    ...
    renderDetail() {
        const { CaseData: { loading } } = this.props;
        const { caseDetails, trailLeft, TrackPaddingBottom1, TrackPaddingTop, TrackPaddingBottom } = this.state;
        ...
        return (
            <div style={{ padding: '24px 0', background: '#252C3C', /*height: autoheight() - 280 + 'px'*/ }}
                    ...
                    <Row gutter={rowLayout} className={styles.xqrow}>
                        <Col md={8} sm={24} className={styles.xqcol}>
                            ...
                            <div className={liststyles.Indextail} style={{ paddingLeft: 102 }}>
                                <div className={liststyles.special1}><Ellipsis lines={1} tooltip>{caseDetails && caseDetails.ajmc ? caseDetails.ajmc : ''}</Ellipsis>
                                </div>
                            </div>
                        </Col>
                        <Col md={8} sm={24} className={styles.xqcol}>
                    ...
                    <Row gutter={rowLayout} className={styles.xqrow}>
                        <Col md={24} sm={24} className={styles.xqcol}>
                            ...
                            <div className={liststyles.Indextail}
                                 style={{ paddingLeft: 84 }}>{caseDetails && caseDetails.jyaq ? caseDetails.jyaq : ''}</div>
                        </Col>
                    </Row>
                    {caseDetails && caseDetails.pajk ?
                        <Row gutter={rowLayout} className={styles.xqrow}>
                            <Col md={24} sm={24} className={styles.xqcol}>
                                ...
                                <div className={liststyles.Indextail}
                                     style={{ paddingLeft: 84 }}>{caseDetails && caseDetails.pajk ? caseDetails.pajk : ''}</div>
                            </Col>
                        </Row>
                        :
                        ''
                    ...
                    {caseDetails && caseDetails.xayy ?
                        <Row gutter={rowLayout} className={styles.xqrow}>
                            <Col md={24} sm={24} className={styles.xqcol}>
                                ...
                                <div className={liststyles.Indextail}
                                     style={{ paddingLeft: 84 }}>{caseDetails && caseDetails.xayy ? caseDetails.xayy : ''}</div>
                            </Col>
                        </Row>
                        :
                        ''
                ...
                <div style={{ textAlign: 'center' }}>
                    {caseDetails && caseDetails.jqxxList && caseDetails.jqxxList.length > 0 ?
                        <Button type='primary' onClick={() => this.seePolice(true, caseDetails.jqxxList)}
                                style={{ marginRight: 16 }}>查看关联警情</Button>
                        :
                        ''
                    }
                    {caseDetails && caseDetails.rqxyrList && caseDetails.rqxyrList.length > 0 ?
                        <Button type='primary' onClick={() => this.seeArea(true, caseDetails.rqxyrList)}
                                style={{ marginRight: 16 }}>查看涉案人员在区情况</Button>
                        :
                        ''
                    }
                    {caseDetails && caseDetails.sawpList && caseDetails.sawpList.length > 0 ?
                        <Button type='primary' onClick={() => this.seeRes(true, caseDetails.sawpList)}
                                style={{ marginRight: 16 }}>查看涉案物品</Button>
                        :
                        ''
                    }
                    {caseDetails && caseDetails.jzList && caseDetails.jzList.length > 0 ?
                        <Button type='primary' onClick={() => this.seeDossier(true, caseDetails.jzList)}
                                style={{ marginRight: 16 }}>查看卷宗信息</Button>
                        :
                        ''
                    }
                </div>
    ...
    render() {
        const { superviseVisibleModal, caseDetails, policevisible, resvisible, areavisible, Dossiervisible, RetrieveVisible, makeTableModalVisible,tbDetail } = this.state;
        let detail = (
            <Row style={{ width: '90%', margin: '0 38px 10px', lineHeight: '36px', color: 'rgba(0, 0, 0, 0.85)' }}>
                <Col span={12}>案件名称：<Tooltip
                    title={caseDetails && caseDetails.ajmc && caseDetails.ajmc.length > 20 ? caseDetails.ajmc : null}>{caseDetails && caseDetails.ajmc ? caseDetails.ajmc.length > 20 ? caseDetails.ajmc.substring(0, 20) + '...' : caseDetails.ajmc : ''}</Tooltip></Col>
                <Col span={12}>办案单位：<Tooltip
                    title={caseDetails && caseDetails.sadwmc && caseDetails.sadwmc.length > 20 ? caseDetails.sadwmc : null}>{caseDetails && caseDetails.sadwmc ? caseDetails.sadwmc.length > 20 ? caseDetails.sadwmc.substring(0, 20) + '...' : caseDetails.sadwmc : ''}</Tooltip></Col>
