...
export function getTimeDistance(type) {
  ...
  if (type === 'beforeLastDay') {
    return [moment().subtract(2, 'days').startOf('day'), moment().subtract(2, 'days').endOf('day')];
  }
  // 上周
  if (type === 'lastWeek') {
  ...
  if (type === 'beforeLastWeek') {
    return [moment().subtract(2, 'weeks').startOf('week'), moment().subtract(2, 'weeks').endOf('week')];
  }
  // 上月
  if (type === 'lastMonth') {
  ...
  if (type === 'beforeLastMonth') {
    return [moment().subtract(2, 'months').startOf('month'), moment().subtract(2, 'months').endOf('month')];
  }
}
...
// 推送类型(督办、预警、告警)
export const pushTypeDictCode = { SUPERVISION: '5009673', EARLYWARNING: '5009672', WARNING: '5009671' };
